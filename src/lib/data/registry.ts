...
const eventDetailsById = new Map([...(eventDetailsV2Data as EventRow[]), ...(eventDetailsBatchAData as EventRow[]), ...(eventDetailsBatchBData as EventRow[]), ...(eventDetailsBatchCData as EventRow[]), ...(eventDetailsIssuerControl2026Data as EventRow[])].map((row) => [row.id, row] as const));
const stablecoins = [...(stablecoinsData as StablecoinRow[]), ...(stablecoinsExtraData as StablecoinRow[]), ...(stablecoinsBatchBData as StablecoinRow[]), ...(stablecoinsBatchCData as StablecoinRow[])].map((coin) => ({ ...coin, ...(stablecoinOverridesById.get(coin.id) ?? {}), ...(classificationById.get(coin.id) ?? {}), ...(classificationExtensionById.get(coin.id) ?? {}), ...(getStablecoinProfile(coin.id) ?? {}) }));
const organizations = [...(organizationsData as OrganizationRow[]), ...(organizationsBatchBData as OrganizationRow[]), ...(organizationsBatchCData as OrganizationRow[])].map((organization) => ({ ...organization, issuer_type: organization.legacy_issuer_type ?? organization.organization_type }));
const relationships = [...(relationshipsData as RelationshipRow[]), ...(relationshipsBatchBData as RelationshipRow[]), ...(relationshipsBatchCData as RelationshipRow[])];
const events = [...(eventsData as EventRow[]), ...(eventsPr036Data as EventRow[]), ...(eventsPr037Data as EventRow[]), ...(eventsPr038Data as EventRow[]), ...(eventsBatchAData as EventRow[]), ...(eventsBatchBData as EventRow[]), ...(eventsBatchCData as EventRow[]), ...(eventsIssuerControl2026Data as EventRow[])].map((event) => ({ ...event, ...(eventDetailsById.get(event.id) ?? {}) }));
const evidence = [...(evidenceData as EvidenceRow[]), ...(evidenceExtraData as EvidenceRow[]), ...(evidencePr033Data as EvidenceRow[]), ...(evidenceEventsPr036Data as EvidenceRow[]), ...(evidenceEventsPr037Data as EvidenceRow[]), ...(evidenceEventsPr038Data as EvidenceRow[]), ...(evidenceBatchAData as EvidenceRow[]), ...(evidenceBatchBData as EvidenceRow[]), ...(evidenceBatchCData as EvidenceRow[]), ...(evidenceIssuerControl2026Data as EvidenceRow[])].map(withEvidenceV2Fields);
...
const deployments = [...(deploymentsData as DeploymentRow[]), ...(deploymentsExtraData as DeploymentRow[]), ...(deploymentsBatchAData as DeploymentRow[]), ...(deploymentsBatchBData as DeploymentRow[]), ...(deploymentsBatchCData as DeploymentRow[]), ...(deploymentsIssuerControl2026Data as DeploymentRow[])];
