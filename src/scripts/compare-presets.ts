...
type ComparePresetConfig = { presets: ComparePreset[] };
type CompareDimensionGroup = { id: string; dimensions: Array<{ id: string }> };
type CompareDimensionConfig = { groups: CompareDimensionGroup[] };
...
if (presetRoot instanceof HTMLElement) {
  ...
  const presetConfig: ComparePresetConfig = presetConfigElement?.textContent ? JSON.parse(presetConfigElement.textContent) as ComparePresetConfig : { presets: [] };
  const dimensionConfig: CompareDimensionConfig = dimensionConfigElement?.textContent ? JSON.parse(dimensionConfigElement.textContent) as CompareDimensionConfig : { groups: [] };
  ...
  const updatePresetStatus = (message: string) => { if (presetStatus) presetStatus.textContent = message; };
  ...
  function visibleFacetCount(groupIds: string[]) { return groupIds.reduce((sum, groupId) => sum + (dimensionCountByGroup.get(groupId) ?? 0), 0); }
  function syncRenderedGroupVisibility() {
    const selected = new Set(selectedGroupIds());
    for (const section of root.querySelectorAll<HTMLElement>('[data-compare-group]')) section.hidden = !selected.has(section.dataset.compareGroup ?? '');
  }
  ...
  function setPressedPreset(presetId: string | null) {
    for (const button of presetButtons) button.setAttribute('aria-pressed', button.dataset.comparePresetId === presetId ? 'true' : 'false');
  }
  function selectionMatchesPreset(preset: ComparePreset, groupIds = selectedGroupIds()) {
    return JSON.stringify(selectedAssetSlugs()) === JSON.stringify(preset.asset_slugs)
      && JSON.stringify(groupIds) === JSON.stringify(normalizedGroups(preset.visible_group_ids));
  }
  function replaceUrlPresetState(presetId: string | null, groupIds: string[]) {
    ...
    if (normalized.length === allGroupIds.length) params.delete('groups'); else params.set('groups', normalized.join(','));
  ...
  function pushGroupUrl(groupIds: string[]) {
    ...
    const normalized = normalizedGroups(groupIds);
    if (normalized.length === allGroupIds.length) params.delete('groups'); else params.set('groups', normalized.join(','));
    window.history.pushState({}, '', `${window.location.pathname}${params.size ? `?${params.toString()}` : ''}${window.location.hash}`);
  }
  function setSlotValues(assetSlugs: string[]) { slots.forEach((slot, index) => { slot.value = assetSlugs[index] ?? ''; }); }
  ...
  function applyPreset(preset: ComparePreset, historyMode: 'push' | 'replace' = 'push') {
    ...
    if (historyMode === 'push') {
      ...
      if (groups.length === allGroupIds.length) params.delete('groups'); else params.set('groups', groups.join(','));
  ...
  function applyUrlPresetState() {
    ...
    if (requestedPreset && explicitAssets.length === 0) { applyPreset(requestedPreset, 'replace'); return; }
  ...
  for (const button of presetButtons) button.addEventListener('click', () => {
    ...
    if (preset) applyPreset(preset, 'push');
  });
  ...
  for (const toggle of groupToggles) toggle.addEventListener('change', () => {
    if (selectedGroupIds().length === 0) {
      toggle.checked = true;
      updatePresetStatus('At least one facet group must remain visible.');
      return;
    }
    activePresetId = null;
    setPressedPreset(null);
