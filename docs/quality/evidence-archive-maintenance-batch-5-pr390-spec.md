...
Manually review the ten PR #388 Queue v4 Evidence identities and apply only source-preserving archive maintenance supported by exact capture or reviewed source-equivalence evidence.
...
The initial probe phase must commit only `docs/migration/evidence-archive-maintenance-batch-5-pr390-review-queue.json`; canonical writes remain disabled until reviewed decisions are added.
...
PR #390 may change at most ten existing Evidence rows. It may add no identity, remove no identity, and change no Evidence Relation.
