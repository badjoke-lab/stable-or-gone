export const legacyProjectedEvidenceIds = new Set([
  'sog_src_tether_transparency',
  'sog_src_tether_assurance_opinions',
  'sog_src_tether_legal_terms',
  'sog_src_circle_transparency',
  'sog_src_circle_svb_update',
  'sog_src_circle_usdc_product',
  'sog_src_circle_stability_update_2023_03_13',
  'sog_src_circle_reserve_reports',
  'sog_src_circle_usdc_2026_01_examination_report',
  'sog_src_circle_usdc_contract_addresses',
  'sog_src_circle_usdc_redemption_access_faq',
  'sog_src_makerdao_docs_dai',
  'sog_src_sky_dai_product_docs',
  'sog_src_makerdao_forum_lifecycle_reference',
  'sog_src_terra_docs',
  'sog_src_ust_sec_2023_32',
  'sog_src_ust_failure_research_arxiv',
  'sog_src_ust_luna_classic_recovery_arxiv',
  'sog_src_paxos_busd_announcement',
  'sog_src_busd_paxos_issuer_update',
  'sog_src_busd_binance_phaseout',
  'sog_src_busd_reuters_sec_2024',
  'sog_src_frax_docs_frax',
  'sog_src_frax_official_site',
  'sog_src_tusd_official_site',
  'sog_src_tusd_attestations',
  'sog_src_fdusd_official_site',
  'sog_src_fdusd_reserve_reports',
  'sog_src_pyusd_paypal_official',
  'sog_src_pyusd_paxos_official',
  'sog_src_usdd_official_site',
  'sog_src_tron_dao_reserve_site',
  'sog_src_gusd_gemini_official',
  'sog_src_lusd_liquity_docs',
  'sog_src_crvusd_curve_docs',
  'sog_src_usde_ethena_docs',
  'sog_src_susd_synthetix_docs',
  'sog_src_rlusd_ripple_page',
  'sog_src_rlusd_transparency_reports',
  'sog_src_rlusd_user_terms',
  'sog_src_rlusd_docs',
  'sog_src_rlusd_implementation_repo',
  'sog_src_eurc_circle_page',
  'sog_src_eurc_transparency',
  'sog_src_eurc_mint_page',
  'sog_src_eurc_developer_docs',
  'sog_src_usdp_paxos_page',
  'sog_src_usdp_transparency',
  'sog_src_usdp_etherscan',
  'sog_src_usdp_contracts_github',
  'sog_src_usdp_solscan',
  'sog_src_usdg_global_dollar_page',
  'sog_src_usdg_paxos_page',
  'sog_src_usdg_network_terms',
  'sog_src_usds_sky_page',
  'sog_src_usds_susds_page',
  'sog_src_usds_developer_docs',
  'sog_src_usds_sky_github',
  'sog_src_usds_info_dashboard',
  'sog_src_husd_etherscan',
  'sog_src_rlusd_launch_2024',
  'sog_src_eurc_launch_2022',
  'sog_src_frax_docs',
  'sog_src_frax_app',
  'sog_src_tusd_site',
  'sog_src_tusd_attestation_entry',
  'sog_src_fdusd_site',
  'sog_src_fdusd_reserve_entry',
  'sog_src_pyusd_paypal_page',
  'sog_src_pyusd_paxos_page',
  'sog_src_pyusd_transparency',
  'sog_src_usdd_site',
  'sog_src_usdd_tron_dao_reserve',
  'sog_src_usdt_cftc_2021_event',
  'sog_src_usdt_nyag_2021_event',
  'sog_src_usdc_reopen_2023_event',
  'sog_src_dai_mcd_2019_event',
  'sog_src_dai_black_thursday_event',
  'sog_src_dai_sky_transition_event',
  'sog_src_ust_lfg_research_event',
  'sog_src_ust_chain_halt_research_event',
  'sog_src_ust_sec_event',
  'sog_src_busd_binance_phaseout_event',
  'sog_src_fdusd_firstdigital_launch_event',
  'sog_src_fdusd_binance_adoption_event',
  'sog_src_pyusd_paypal_launch_event',
  'sog_src_pyusd_solana_event',
  'sog_src_usdd_official_launch_event',
  'sog_src_gusd_gemini_launch_event',
  'sog_src_lusd_liquity_lifecycle_event',
  'sog_src_crvusd_curve_lifecycle_event',
  'sog_src_usde_ethena_lifecycle_event',
  'sog_src_susd_synthetix_lifecycle_event',
  'sog_src_usde_public_launch_2024',
]);

export function getEvidenceRelationOrigin(evidenceId) {
  return legacyProjectedEvidenceIds.has(evidenceId) ? 'legacy_subject_projection' : 'explicit_v2';
}
