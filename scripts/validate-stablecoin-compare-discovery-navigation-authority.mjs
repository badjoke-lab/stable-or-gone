...
check(authority.canonical_baseline?.archive_recorded === 463 && authority.canonical_baseline?.archive_not_recorded === 122, 'archive baseline');
...
for (const marker of ['persistent Compare dock', 'Add / replace record', 'REVIEW_GATE']) check(amendment.includes(marker), `amendment missing ${marker}`);
for (const marker of ['comparison panel must precede', 'Compare dock', 'Add / replace record', 'No page-level horizontal overflow']) check(spec.includes(marker), `spec missing ${marker}`);
for (const body of [agents, roadmap, governance, deployment]) check(body.includes('Stablecoin Compare Discovery and Navigation Remediation'), 'forward governance missing Compare discovery authority');
check(agents.includes('Canonical stable assets: 119') && agents.includes('Canonical Evidence: 585'), 'AGENTS canonical markers');
check(agents.includes('docs/ui-v3-remediation-authority.md') && agents.includes('PR #493') && agents.includes('PR #500') && agents.includes('PR #517'), 'AGENTS historical regression markers');
...
console.log(JSON.stringify({
  ...
  stage: 'AUTHORIZED_UI_REMEDIATION',
