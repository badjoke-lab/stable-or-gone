...
const paths = {
  config: 'config/evidence-archive-maintenance-batch-3-pr380.json',
  decisions: 'config/evidence-archive-maintenance-batch-3-pr380-decisions.json',
  sourceQueue: 'docs/migration/evidence-archive-maintenance-queue-v2-pr378.json',
  probeQueue: 'docs/migration/evidence-archive-maintenance-batch-3-pr380-review-queue.json',
  authority: 'docs/migration/post-pr378-review-gate-pr379.json',
...
const canonicalCheckpointId = 'sog_evidence_archive_maintenance_batch_3_canonical_112_checkpoint_pr380_2026_07_16';
const statsCheckpointId = 'sog_evidence_archive_maintenance_batch_3_112_checkpoint_pr380_2026_07_16';
...
function readBaseText(file) {
  try {
    return execFileSync('git', ['show', `${baseRef}:${file}`], { cwd: root, encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'] });
  } catch (error) {
    if (process.env.SOG_PR380_ALLOW_DISK_BASE === '1') return readText(file);
...
function assertAuthority(config, authority, queue) {
  const decision = authority.decisions?.evidence_archive_maintenance_batch_3;
  if (decision?.pr !== 380 || decision?.decision !== 'approved_bounded') throw new Error('PR #379 does not authorize PR #380');
  if (queue.queue_id !== 'sog_evidence_archive_maintenance_queue_v2_pr378') throw new Error(`Unexpected source queue ${queue.queue_id}`);
  const selected = queue.selected_candidates.map((row) => row.evidence_id);
  if (JSON.stringify(selected) !== JSON.stringify(config.selected_evidence_ids)) throw new Error('PR #380 selected IDs differ from PR #378 queue');
  if (JSON.stringify(decision.selected_evidence_ids) !== JSON.stringify(config.selected_evidence_ids)) throw new Error('PR #379 selected authority differs from PR #380 config');
}

function validateDecisions(config, decisionFile, probeQueue, queue) {
  const allowed = new Set(config.required_outcomes);
  ...
  if (JSON.stringify(decisions.map((row) => row.evidence_id)) !== JSON.stringify(selected)) throw new Error('Decision order or identity differs from selected queue');
  ...
  for (const decision of decisions) {
    ...
    if (!probe || !candidate) throw new Error(`${decision.evidence_id}: missing probe or source queue row`);
    if (decision.canonical_url !== candidate.url || decision.canonical_url !== probe.canonical_url) throw new Error(`${decision.evidence_id}: canonical URL mismatch`);
    if (decision.outcome === 'dated_exact_archive_added') {
      ...
      if (!match) throw new Error(`${decision.evidence_id}: accepted capture is absent from exact CDX probe`);
      if (match.statuscode !== '200') throw new Error(`${decision.evidence_id}: accepted capture is not HTTP 200`);
      if (match.original !== decision.canonical_url) throw new Error(`${decision.evidence_id}: accepted capture is not the exact canonical source URL`);
      const expectedArchive = `https://web.archive.org/web/${decision.capture_timestamp}/${decision.canonical_url}`;
      if (decision.archived_url !== expectedArchive) throw new Error(`${decision.evidence_id}: archived URL does not match accepted capture`);
    }
    if (decision.outcome === 'reviewed_source_replacement') {
      if (!decision.replacement_url || probe.live_probe?.final_url !== decision.replacement_url) throw new Error(`${decision.evidence_id}: replacement is not the reviewed live redirect target`);
      if (!probe.live_probe?.redirected || !probe.live_probe?.ok) throw new Error(`${decision.evidence_id}: replacement redirect was not successful`);
      if (decision.claim_scope_equivalence !== 'reviewed_equivalent' || !decision.source_version_equivalence?.startsWith('reviewed_')) throw new Error(`${decision.evidence_id}: source equivalence review is incomplete`);
    }
  }
  return { probes, queued };
}
...
function buildCanonicalFiles(queue, decisions) {
  ...
  for (const file of sourceFiles) {
    ...
    outputs[file] = rows.map((row) => {
      ...
      if (row.url !== decision.canonical_url) throw new Error(`${row.id}: base canonical URL changed before PR #380`);
      if (String(row.archived_url ?? '').trim()) throw new Error(`${row.id}: base row already has an archive`);
...
function buildCheckpoint(baseCheckpoint, decisions) {
  ...
  return {
    ...
    source_commit: 'pr380-evidence-archive-maintenance-batch-3',
    asset_count: 112,
    source_checkpoint_id: baseCheckpoint.checkpoint_id,
    previous_checkpoint_id: baseCheckpoint.checkpoint_id,
    maintenance_pr: 380,
    expected_counts: { ...baseCheckpoint.expected_counts },
    evidence_quality: {
      archive_index_count: baseCheckpoint.evidence_quality.archive_index_count + archiveAdded.length,
      archive_not_recorded_count: baseCheckpoint.evidence_quality.archive_not_recorded_count - archiveAdded.length,
    ...
    notes: 'Current deterministic canonical checkpoint after PR #380 Evidence and Archive Maintenance Batch 3. Nine exact dated archive captures and one reviewed claim-scope-equivalent Circle Mint route replacement were accepted across the ten selected Evidence identities. All canonical identity counts, Evidence Relations, assets, Market Access records, non-Evidence record families, and public surfaces remain unchanged.'
  };
}

function buildStatsCheckpoint(baseStatsCheckpoint) {
  return {
    schema_version: '1.0',
    status: 'reviewed_non_growth_checkpoint',
    ...
    checkpoint_kind: 'non_growth_normalization_checkpoint',
    recorded_at: '2026-07-16',
    registry_version: 'pr380-evidence-archive-maintenance-batch-3',
    asset_count: 112,
    source_checkpoint_id: baseStatsCheckpoint.checkpoint_id,
    canonical_checkpoint_id: canonicalCheckpointId,
    previous_history_checkpoint_id: baseStatsCheckpoint.checkpoint_id,
    maintenance_pr: 380,
    notes: 'Reviewed forward same-count statistics checkpoint for PR #380 Evidence and Archive Maintenance Batch 3. It records archive coverage 399 of 559 while preserving every canonical record count and public-surface boundary.'
  };
}

function buildReleaseBaseline(baseBaseline) {
  return {
    ...baseBaseline,
    status: 'current',
    baseline_id: 'sog_release_integrity_pr380_112_assets_2026_07_16',
    recorded_at: '2026-07-16',
    source_checkpoint_commit: 'pr380-evidence-archive-maintenance-batch-3',
    evidence_quality: {
      archive_index_count: 399,
      archive_not_recorded_count: 160,
...
function buildOutcomes(queue, decisions, canonicalFiles) {
  ...
  return {
    schema_version: '1.0',
    outcome_id: 'sog_evidence_archive_maintenance_outcomes_pr380_2026_07_16',
    ...
    archive_index_count_before: 390,
    archive_index_count_after: 399,
    archive_not_recorded_count_before: 169,
    archive_not_recorded_count_after: 160,
...
export function buildEvidenceArchiveMaintenanceBatch3Outputs() {
  ...
  const outcomes = buildOutcomes(queue, decisions, canonicalFiles);
  ...
  const handoffBase = {
    schema_version: '1.0',
    handoff_id: 'sog_evidence_archive_maintenance_batch_3_pr380_reviewed_handoff_2026_07_16',
    status: 'reviewed_complete',
    review_pr: 380,
    source_pr: 379,
    source_merge_commit: '1dfb8c7c5a338488c89007223fe3bac277164b49',
    queue_id: queue.queue_id,
    ...
    evidence_quality: {
      archive_recorded: 399,
      archive_not_recorded: 160,
      selected: 10,
      changed: 10,
      dated_archive_added: 9,
      reviewed_source_replacement: 1,
      reviewed_no_safe_change: 0
    },
    changed_evidence_ids: decisions.map((row) => row.evidence_id),
    dated_archive_added_evidence_ids: decisions.filter((row) => row.outcome === 'dated_exact_archive_added').map((row) => row.evidence_id),
    source_replacement_evidence_ids: decisions.filter((row) => row.outcome === 'reviewed_source_replacement').map((row) => row.evidence_id),
    reviewed_no_safe_change_evidence_ids: decisions.filter((row) => row.outcome === 'reviewed_no_safe_change').map((row) => row.evidence_id),
    ...
    next_work_item: {
      ...
      reason: 'Merged PR #379 authorizes only PR #380 before another review gate; no later archive, dossier, Market Access, growth, or public-surface work is pre-authorized.'
    },
    boundaries: {
      new_asset: false,
...
if (isCli) {
  const result = buildEvidenceArchiveMaintenanceBatch3Outputs();
  if (process.argv.includes('--check')) {
    const stale = Object.entries(result.files).filter(([file, value]) => readText(file) !== serialize(value)).map(([file]) => file);
    if (stale.length) {
      console.error(`PR #380 outputs are not reproducible: ${stale.join(', ')}`);
      process.exit(1);
    }
  }
