...
const promotionFiles = ['data/candidate-promotions-batch-d.json', 'data/candidate-promotions-batch-f.json', 'data/candidate-promotions-batch-g.json'];
...
for (const file of promotionFiles) {
  ...
  const rows = read(file);
  if (!Array.isArray(rows)) failures.push(`${file}: expected array`);
  for (const row of rows) {
    if (!row.candidate_id) failures.push(`${file}: missing candidate_id`);
    if (patches.has(row.candidate_id)) failures.push(`duplicate promotion patch: ${row.candidate_id}`);
...
for (const row of [...relationships, ...events, ...evidence]) {
  if (row.stablecoin_id) referencedIds.add(row.stablecoin_id);
  for (const field of ['stablecoin_ids','subject_stablecoin_ids']) for (const id of row[field] ?? []) referencedIds.add(id);
}
...
for (const candidate of candidates) {
  ...
  const canonicalBySlug = stablecoinBySlug.get(slug);
  if (candidate.status === 'promoted') {
    if (!canonical) failures.push(`${id}: promoted candidate lacks canonical record`);
    else {
      if (canonical.slug !== candidate.slug) failures.push(`${id}: canonical slug mismatch`);
      if (normalize(canonical.name) !== normalize(candidate.name)) failures.push(`${id}: canonical name mismatch (${candidate.name} vs ${canonical.name})`);
    ...
    if (canonical || canonicalBySlug || classificationIds.has(candidate.proposed_record_id) || profileIds.has(candidate.proposed_record_id) || referencedIds.has(candidate.proposed_record_id)) failures.push(`${id}: non-promoted candidate collides with canonical data`);
  }
...
for (const stablecoin of stablecoins) {
  const candidate = candidateByRecordId.get(stablecoin.id);
  if (!candidate) failures.push(`canonical stablecoin missing from candidate controls: ${stablecoin.id}`);
  else if (candidate.status !== 'promoted') failures.push(`canonical stablecoin not effectively promoted: ${stablecoin.id}`);
}
