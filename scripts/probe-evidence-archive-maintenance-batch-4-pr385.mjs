...
const queuePath = 'docs/migration/evidence-archive-maintenance-queue-v3-pr383.json';
const configPath = 'config/evidence-archive-maintenance-batch-4-pr385.json';
const outputPath = 'docs/migration/evidence-archive-maintenance-batch-4-pr385-review-queue.json';
...
if (JSON.stringify(selectedIds) !== JSON.stringify(config.selected_evidence_ids)) {
  throw new Error('PR #385 config selected IDs do not match PR #383 queue order');
}
...
const output = {
  schema_version: '1.0',
  queue_id: 'sog_evidence_archive_maintenance_batch_4_pr385_review_queue',
