...
import { buildEvidenceArchiveMaintenanceBatch6Outputs } from './build-evidence-archive-maintenance-batch-6-pr395.mjs';
...
const decisionsFile = readJson('config/evidence-archive-maintenance-batch-6-pr395-decisions.json');
const sourceQueue = readJson('docs/migration/evidence-archive-maintenance-queue-v5-pr393.json');
const authority = readJson('docs/migration/post-pr393-review-gate-pr394.json');
const reviewQueue = readJson('docs/migration/evidence-archive-maintenance-batch-6-pr395-review-queue.json');
...
expect(authority.decisions?.evidence_archive_maintenance_batch_6?.pr === 395, 'PR #394 authority changed');
...
expect(same(sourceQueue.selected_candidates.map((row) => row.evidence_id), selected), 'Queue v5 selected IDs differ from config');
expect(same(authority.decisions.evidence_archive_maintenance_batch_6.selected_evidence_ids, selected), 'authority selected IDs differ from config');
...
expect(archiveIds.length === 9 && replacementIds.length === 0 && noSafeIds.length === 1, 'review outcome mix changed');
expect(same(noSafeIds, ['sog_src_makerdao_docs_dai']), 'no-safe-change identity changed');
...
expect(reviewQueue.outcome_counts.dated_exact_archive_added === 9, 'dated archive count changed');
expect(reviewQueue.outcome_counts.reviewed_source_replacement === 0, 'source replacement count changed');
expect(reviewQueue.outcome_counts.reviewed_no_safe_change === 1, 'no-safe-change count changed');

expect(outcomes.selected_count === 10, 'outcome selected count changed');
expect(outcomes.changed_count === 9, 'outcome changed count changed');
expect(outcomes.dated_archive_added_count === 9, 'outcome archive count changed');
expect(outcomes.reviewed_source_replacement_count === 0, 'outcome replacement count changed');
expect(outcomes.reviewed_no_safe_change_count === 1, 'outcome no-safe count changed');
...
expect(outcomes.archive_index_count_before === 416 && outcomes.archive_index_count_after === 425, 'archive coverage transition changed');
expect(outcomes.archive_not_recorded_count_before === 143 && outcomes.archive_not_recorded_count_after === 134, 'archive gap transition changed');
expect(same(outcomes.outcomes.map((row) => row.evidence_id), selected), 'outcome identities changed');
expect(same([...outcomes.changed_files].sort(), ['data/evidence-batch-a.json', 'data/evidence-batch-b.json', 'data/evidence-batch-d.json', 'data/evidence.json']), 'canonical changed-file boundary changed');

expect(checkpoint.checkpoint_id === 'sog_evidence_archive_maintenance_batch_6_canonical_112_checkpoint_pr395_2026_07_16', 'canonical checkpoint ID changed');
expect(checkpoint.source_checkpoint_id === 'sog_evidence_archive_maintenance_batch_5_canonical_112_checkpoint_pr390_2026_07_16', 'canonical checkpoint ancestry changed');
...
expect(checkpoint.evidence_quality.canonical_changes === 9, 'checkpoint changed count changed');
expect(checkpoint.evidence_quality.reviewed_no_safe_change === 1, 'checkpoint no-safe count changed');
...
expect(statsCheckpoint.checkpoint_id === 'sog_evidence_archive_maintenance_batch_6_112_checkpoint_pr395_2026_07_16', 'stats checkpoint ID changed');
expect(statsCheckpoint.canonical_checkpoint_id === checkpoint.checkpoint_id, 'stats/canonical checkpoint binding changed');
expect(statsCheckpoint.maintenance_pr === 395, 'stats maintenance PR changed');
expect(releaseBaseline.baseline_id === 'sog_release_integrity_pr395_112_assets_2026_07_16', 'release baseline ID changed');
...
expect(releaseBaseline.evidence_quality.archive_index_count === 425, 'release archive count changed');
expect(releaseBaseline.evidence_quality.archive_not_recorded_count === 134, 'release no-archive count changed');
expect(releaseBaseline.evidence_quality.reviewed_no_safe_change_assets === 1, 'release no-safe count changed');

expect(handoff.status === 'reviewed_complete' && handoff.review_pr === 395, 'handoff status changed');
...
expect(handoff.evidence_quality.archive_recorded === 425 && handoff.evidence_quality.archive_not_recorded === 134, 'handoff archive boundary changed');
expect(handoff.evidence_quality.selected === 10 && handoff.evidence_quality.changed === 9, 'handoff review boundary changed');
expect(handoff.evidence_quality.dated_archive_added === 9, 'handoff archive outcome count changed');
expect(handoff.evidence_quality.reviewed_source_replacement === 0, 'handoff replacement outcome count changed');
expect(handoff.evidence_quality.reviewed_no_safe_change === 1, 'handoff no-safe outcome count changed');
expect(same(handoff.changed_evidence_ids, changedIds), 'handoff changed IDs changed');
expect(handoff.next_work_item?.decision === 'review_gate_required', 'PR #395 must end at review gate');
...
if (originMainAvailable) {
  ...
  const sourceFiles = ['data/evidence-batch-a.json', 'data/evidence-batch-b.json', 'data/evidence-batch-d.json', 'data/evidence.json'];
  const seen = new Set();
...
for (const [file, markers] of [
  ['AGENTS.md', ['PR #395 Evidence and Archive Maintenance Batch 6: active; complete on merge', 'dated_exact_archive_added: 9', 'reviewed_no_safe_change: 1', 'PR #395 must stop at `REVIEW GATE`']],
  ['docs/roadmap.md', ['Status: canonical execution schedule — PR #395 active', 'Archive recorded after reviewed decisions: 425', 'Archive not recorded after reviewed decisions: 134', 'After PR #395, stop at `REVIEW GATE`']],
  ['docs/quality/evidence-archive-maintenance-batch-6-pr395-spec.md', ['nine dated exact archive additions', 'one reviewed no-safe-change', 'PR #395 stops at `REVIEW GATE`']],
  ['docs/roadmap-amendments/2026-07-16-pr395-evidence-archive-maintenance-batch-6-activation.md', ['nine exact dated archives', 'one reviewed no-safe-change', 'stop at `REVIEW GATE`']]
]) {
...
for (const file of [
  'public/data/evidence-archive-maintenance-outcomes-pr395.json',
  'public/data/evidence-archive-maintenance-batch-6-pr395-reviewed-handoff.json',
...
if (failures.length) {
  console.error('PR #395 Evidence and Archive Maintenance Batch 6 validation failed:');
