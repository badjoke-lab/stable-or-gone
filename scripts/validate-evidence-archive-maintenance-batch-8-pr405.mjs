...
import { buildEvidenceArchiveMaintenanceBatch8Outputs } from './build-evidence-archive-maintenance-batch-8-pr405.mjs';

const root = process.cwd();
const baseRef = process.env.SOG_PR405_BASE_REF ?? 'origin/main';
...
const decisionsFile = readJson('config/evidence-archive-maintenance-batch-8-pr405-decisions.json');
const sourceQueue = readJson('docs/migration/evidence-archive-maintenance-queue-v7-pr403.json');
const authority = readJson('docs/migration/post-pr403-review-gate-pr404.json');
const reviewQueue = readJson('docs/migration/evidence-archive-maintenance-batch-8-pr405-review-queue.json');
...
const result = buildEvidenceArchiveMaintenanceBatch8Outputs();
...
const changedIds = decisions.map((row) => row.evidence_id);
...
expect(authority.decisions?.evidence_archive_maintenance_batch_8?.pr === 405, 'PR #404 authority changed');
expect(authority.decisions?.evidence_archive_maintenance_batch_8?.decision === 'approved_bounded_manual_review', 'PR #404 Batch 8 decision changed');
expect(same(sourceQueue.selected_candidates.map((row) => row.evidence_id), selected), 'Queue v7 selected IDs differ from config');
expect(same(authority.decisions.evidence_archive_maintenance_batch_8.selected_evidence_ids, selected), 'authority selected IDs differ from config');
...
expect(same(archiveIds, selected), 'archive-added identities changed');
expect(replacementIds.length === 0, 'source replacement count changed');
expect(noSafeIds.length === 0, 'no-safe-change count changed');
...
expect(reviewQueue.outcome_counts.dated_exact_archive_added === 10, 'review queue archive count changed');
...
expect(outcomes.selected_count === 10 && outcomes.changed_count === 10, 'outcome selected/changed count changed');
...
expect(outcomes.archive_index_count_before === 430 && outcomes.archive_index_count_after === 440, 'archive coverage transition changed');
expect(outcomes.archive_not_recorded_count_before === 129 && outcomes.archive_not_recorded_count_after === 119, 'archive gap transition changed');
expect(same(outcomes.outcomes.map((row) => row.evidence_id), selected), 'outcome identities changed');
expect(same([...outcomes.changed_files].sort(), ['data/evidence-batch-b.json', 'data/evidence-extra.json']), 'canonical changed-file boundary changed');

expect(checkpoint.checkpoint_id === 'sog_evidence_archive_maintenance_batch_8_canonical_112_checkpoint_pr405_2026_07_17', 'canonical checkpoint ID changed');
expect(checkpoint.source_checkpoint_id === 'sog_evidence_archive_maintenance_batch_7_canonical_112_checkpoint_pr400_2026_07_16', 'canonical checkpoint ancestry changed');
expect(checkpoint.expected_counts.assets === 112 && checkpoint.expected_counts.evidence === 559, 'canonical counts changed');
expect(checkpoint.evidence_quality.archive_index_count === 440, 'checkpoint archive count changed');
expect(checkpoint.evidence_quality.archive_not_recorded_count === 119, 'checkpoint no-archive count changed');
expect(checkpoint.evidence_quality.selected_for_review === 10 && checkpoint.evidence_quality.canonical_changes === 10, 'checkpoint review boundary changed');
...
expect(statsCheckpoint.checkpoint_id === 'sog_evidence_archive_maintenance_batch_8_112_checkpoint_pr405_2026_07_17', 'stats checkpoint ID changed');
expect(statsCheckpoint.canonical_checkpoint_id === checkpoint.checkpoint_id && statsCheckpoint.maintenance_pr === 405, 'stats checkpoint binding changed');
expect(releaseBaseline.baseline_id === 'sog_release_integrity_pr405_112_assets_2026_07_17', 'release baseline ID changed');
...
expect(releaseBaseline.evidence_quality.archive_index_count === 440 && releaseBaseline.evidence_quality.archive_not_recorded_count === 119, 'release archive boundary changed');
expect(releaseBaseline.evidence_quality.reviewed_no_safe_change_assets === 0, 'release no-safe count changed');

expect(handoff.status === 'reviewed_complete' && handoff.review_pr === 405, 'handoff status changed');
...
expect(handoff.evidence_quality.archive_recorded === 440 && handoff.evidence_quality.archive_not_recorded === 119, 'handoff archive boundary changed');
expect(handoff.evidence_quality.selected === 10 && handoff.evidence_quality.changed === 10, 'handoff review boundary changed');
expect(handoff.evidence_quality.dated_archive_added === 10 && handoff.evidence_quality.reviewed_source_replacement === 0 && handoff.evidence_quality.reviewed_no_safe_change === 0, 'handoff outcome mix changed');
...
if (originMainAvailable) {
  ...
  const sourceFiles = ['data/evidence-batch-b.json', 'data/evidence-extra.json'];
  const seen = new Set();
...
for (const [file, markers] of [
  ['AGENTS.md', ['PR #405 Evidence and Archive Maintenance Batch 8: active; complete on merge', 'dated_exact_archive_added: 10', 'reviewed_no_safe_change: 0', 'PR #405 must stop at `REVIEW GATE`']],
  ['docs/roadmap.md', ['Status: canonical execution schedule — PR #405 active', 'Archive recorded after reviewed decisions: 440', 'Archive not recorded after reviewed decisions: 119', 'After PR #405, stop at `REVIEW GATE`']],
  ['docs/quality/evidence-archive-maintenance-batch-8-pr405-spec.md', ['ten dated exact archive additions', 'zero reviewed no-safe-change', 'PR #405 stops at `REVIEW GATE`']],
  ['docs/roadmap-amendments/2026-07-16-pr405-evidence-archive-maintenance-batch-8-activation.md', ['ten exact dated archives', 'zero reviewed no-safe-change', 'stop at `REVIEW GATE`']]
]) {
...
if (failures.length) {
  console.error('PR #405 Evidence and Archive Maintenance Batch 8 validation failed:');
