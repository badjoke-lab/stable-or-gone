...
function inspect(contract) {
  ...
  const selects = [...source.matchAll(/<select\b[^>]*>[\s\S]*?<\/select>/g)].map((match) => {
    ...
    return {
      ...attributes(open),
      options: [...match[0].matchAll(/<option\b[^>]*>([\s\S]*?)<\/option>/g)].map((item) => clean(item[1])).filter(Boolean)
    };
  });
  const headers = [...source.matchAll(/<th\b[^>]*>([\s\S]*?)<\/th>/g)].map((match) => clean(match[1])).filter(Boolean);
  const dataAttributes = unique([...source.matchAll(/\bdata-([a-zA-Z0-9_-]+)/g)].map((match) => match[1]));
  const targetParams = unique([
    contract.search.query_param,
    ...contract.filters.map((filter) => filter.query_param),
    'sort',
    ...(contract.comparison.enabled ? [contract.comparison.query_param] : [])
  ]);
  return {
    id: contract.id,
    ...
    current_behavior: {
      ...
      url_search_params_present: /URLSearchParams|searchParams/.test(source),
      history_replace_present: /history\.replaceState/.test(source),
      history_push_present: /history\.pushState/.test(source),
      popstate_present: /popstate/.test(source),
      clear_all_present: /clear-all|clear filters|reset filters/i.test(source),
      comparison_present: /data-compare|comparison-panel|compare=/i.test(source),
      server_rendered_rows_present: /\.map\(\(/.test(source) || /\.map\(\w+\s*=>/.test(source)
    },
    target_query_parameters: targetParams,
    source_digest: `sha256:${createHash('sha256').update(source).digest('hex')}`
  };
}

const currentImplementation = indexInteractionContracts.map(inspect);
const targetContracts = indexInteractionContracts.map((contract) => ({
  id: contract.id,
  route: contract.route,
  ...
  mobile_row_fields: contract.mobile_row_fields,
...
const implementationGaps = currentImplementation.map((current) => ({
  ...
  note: 'PR 19 records implementation gaps but remains specification-only.'
}));
...
const audit = {
  ...
  implementation_boundary: {
    specification_only: sharedInteractionPolicy.implementation_deferred,
    implementation_starts_at_pr: sharedInteractionPolicy.implementation_starts_at_pr,
    route_changes_allowed: sharedInteractionPolicy.route_changes_allowed
  },
  totals: {
