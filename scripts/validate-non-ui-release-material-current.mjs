...
check(current.asset_count === 116 && c.assets === 116, 'current asset count');
check(c.organizations === 107, 'current organization count');
check(c.relationships === 128, 'current relationship count');
check(c.events === 191, 'current event count');
check(c.evidence === 571, 'current Evidence count');
check(c.market_access_records === 8, 'current Market Access count');
check(c.reserve_reports === 124, 'current reserve-report count');
check(c.known_unknowns === 337, 'current known-unknown count');
check(c.regulatory_notes === 9, 'current regulatory-note count');
check(c.deployments === 182, 'current deployment count');
check(c.legal_profiles === 116, 'current legal-profile count');
check(c.stable_asset_relationships === 5, 'current v3 relationship count');
check(c.reserve_components === 149, 'current reserve-component count');
check(c.income_profiles === 116, 'current income-profile count');
