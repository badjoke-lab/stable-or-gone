...
check(current.asset_count === 114 && c.assets === 114, 'current asset count');
check(c.organizations === 107, 'current organization count');
check(c.relationships === 126, 'current relationship count');
check(c.events === 189, 'current event count');
check(c.evidence === 565, 'current Evidence count');
check(c.market_access_records === 8, 'current Market Access count');
check(c.reserve_reports === 122, 'current reserve-report count');
check(c.known_unknowns === 331, 'current known-unknown count');
check(c.regulatory_notes === 9, 'current regulatory-note count');
check(c.deployments === 180, 'current deployment count');
check(c.legal_profiles === 114, 'current legal-profile count');
check(c.stable_asset_relationships === 5, 'current v3 relationship count');
check(c.reserve_components === 147, 'current reserve-component count');
check(c.income_profiles === 114, 'current income-profile count');
