...
const paths = {
  config: 'config/evidence-archive-maintenance-batch-5-pr390.json',
  decisions: 'config/evidence-archive-maintenance-batch-5-pr390-decisions.json',
  sourceQueue: 'docs/migration/evidence-archive-maintenance-queue-v4-pr388.json',
  probeQueue: 'docs/migration/evidence-archive-maintenance-batch-5-pr390-review-queue.json',
  authority: 'docs/migration/post-pr388-review-gate-pr389.json',
...
const canonicalCheckpointId = 'sog_evidence_archive_maintenance_batch_5_canonical_112_checkpoint_pr390_2026_07_16';
const statsCheckpointId = 'sog_evidence_archive_maintenance_batch_5_112_checkpoint_pr390_2026_07_16';
...
function readBaseText(file) {
  try {
    ...
    if (process.env.SOG_PR390_ALLOW_DISK_BASE === '1') return readText(file);
...
function assertAuthority(config, authority, queue) {
  const decision = authority.decisions?.evidence_archive_maintenance_batch_5;
  if (decision?.pr !== 390 || decision?.decision !== 'approved_bounded') {
    throw new Error('PR #389 does not authorize PR #390');
  }
  if (queue.queue_id !== 'sog_evidence_archive_maintenance_queue_v4_pr388') {
  ...
  if (JSON.stringify(selected) !== JSON.stringify(config.selected_evidence_ids)) {
    throw new Error('PR #390 selected IDs differ from PR #388 Queue v4');
  }
  if (JSON.stringify(decision.selected_evidence_ids) !== JSON.stringify(config.selected_evidence_ids)) {
    throw new Error('PR #389 selected authority differs from PR #390 config');
  }
}
...
function validateDecisions(config, decisionFile, probeQueue, queue) {
  ...
  if (decisions.length !== selected.length) {
    throw new Error(`Expected ${selected.length} decisions, found ${decisions.length}`);
  }
  ...
  for (const decision of decisions) {
    ...
    if (decision.outcome === 'dated_exact_archive_added') {
      ...
      if (!equivalentExactUrl(match.original, decision.canonical_url)) {
        throw new Error(`${decision.evidence_id}: accepted capture is not the exact canonical source identity`);
      }
      const expectedArchive = `https://web.archive.org/web/${decision.capture_timestamp}/${decision.canonical_url}`;
      if (decision.archived_url !== expectedArchive) {
        throw new Error(`${decision.evidence_id}: archived URL does not match accepted capture`);
      }
    }
    if (decision.outcome === 'reviewed_source_replacement') {
      if (!decision.replacement_url || probe.live_probe?.final_url !== decision.replacement_url) {
        throw new Error(`${decision.evidence_id}: replacement is not the reviewed live redirect target`);
      }
      if (!probe.live_probe?.redirected || !probe.live_probe?.ok) {
        throw new Error(`${decision.evidence_id}: replacement redirect was not successful`);
      }
      if (decision.claim_scope_equivalence !== 'reviewed_equivalent' || !decision.source_version_equivalence?.startsWith('reviewed_')) {
        throw new Error(`${decision.evidence_id}: source equivalence review is incomplete`);
      }
    }
  }
}
...
function buildCanonicalFiles(queue, decisions) {
  ...
  for (const file of sourceFiles) {
    ...
    outputs[file] = rows.map((row) => {
      ...
      if (row.url !== decision.canonical_url) throw new Error(`${row.id}: base canonical URL changed before PR #390`);
...
function buildCheckpoint(baseCheckpoint, decisions) {
  ...
  return {
    ...
    source_commit: 'pr390-evidence-archive-maintenance-batch-5',
    asset_count: 112,
...
function buildStatsCheckpoint(baseStatsCheckpoint, checkpoint) {
  return {
    ...
    checkpoint_kind: 'non_growth_normalization_checkpoint',
    recorded_at: '2026-07-16',
    registry_version: 'pr390-evidence-archive-maintenance-batch-5',
    asset_count: 112,
    ...
    maintenance_pr: 390,
    notes: `Reviewed forward same-count statistics checkpoint for PR #390 Evidence and Archive Maintenance Batch 5. It records archive coverage ${checkpoint.evidence_quality.archive_index_count} of 559 while preserving every canonical record count and public-surface boundary.`
  };
}
...
function buildReleaseBaseline(baseBaseline, checkpoint, decisions) {
  return {
    ...baseBaseline,
    status: 'current',
    baseline_id: 'sog_release_integrity_pr390_112_assets_2026_07_16',
    recorded_at: '2026-07-16',
    source_checkpoint_commit: 'pr390-evidence-archive-maintenance-batch-5',
    evidence_quality: {
...
function buildOutcomes(queue, decisions, checkpoint, canonicalFiles) {
  ...
  return {
    schema_version: '1.0',
    outcome_id: 'sog_evidence_archive_maintenance_outcomes_pr390_2026_07_16',
...
export function buildEvidenceArchiveMaintenanceBatch5Outputs() {
  ...
  const handoffBase = {
    schema_version: '1.0',
    handoff_id: 'sog_evidence_archive_maintenance_batch_5_pr390_reviewed_handoff_2026_07_16',
    status: 'reviewed_complete',
    review_pr: 390,
    source_pr: 389,
    source_merge_commit: 'd6c49c90fff5088684d35c2a813dff81bd8f057d',
    queue_id: queue.queue_id,
    ...
    next_work_item: {
      ...
      reason: 'Merged PR #389 authorizes only PR #390 before another review gate; no later archive, dossier, Market Access, growth, or public-surface work is pre-authorized.'
    },
    boundaries: {
      new_asset: false,
...
if (isCli) {
  ...
  if (process.argv.includes('--check')) {
    const stale = Object.entries(result.files)
      ...
      .map(([file]) => file);
    if (stale.length) {
      console.error(`PR #390 outputs are not reproducible: ${stale.join(', ')}`);
      process.exit(1);
    }
  }
