...
const paths = {
  contract: 'config/evidence-archive-review-history-v3-pr387.json',
  authority: 'docs/migration/post-pr385-review-gate-pr386.json',
  ...
  priorManifest: 'docs/migration/evidence-archive-review-history-manifest-v2-pr382.json',
  priorAudit: 'docs/migration/evidence-archive-review-history-audit-v2-pr382.json',
  checkpoint: 'docs/migration/current-canonical-checkpoint.json'
};
const outputPaths = {
  manifest: 'docs/migration/evidence-archive-review-history-manifest-v3-pr387.json',
  audit: 'docs/migration/evidence-archive-review-history-audit-v3-pr387.json'
};
...
function buildHistoryEvents(pr360, pr365, pr380, pr385) {
  const events = [];
...
export function buildEvidenceArchiveReviewHistoryV3Outputs() {
  ...
  if (authority.decisions?.evidence_archive_review_history_contract_v3?.pr !== 387) throw new Error('PR #386 does not authorize PR #387');
  if (authority.decisions?.evidence_archive_review_history_contract_v3?.decision !== 'approved_internal') throw new Error('PR #386 History v3 decision changed');
  if (priorManifest.manifest_id !== 'sog_evidence_archive_review_history_manifest_v2_pr382') throw new Error('Unexpected prior manifest');
  if (priorAudit.audit_id !== 'sog_evidence_archive_review_history_audit_v2_pr382_2026_07_16') throw new Error('Unexpected prior audit');

  const currentById = currentEvidenceById();
  const events = buildHistoryEvents(pr360, pr365, pr380, pr385);
  ...
  const manifest = {
    schema_version: '3.0',
    manifest_id: 'sog_evidence_archive_review_history_manifest_v3_pr387',
  ...
  const audit = {
    schema_version: '3.0',
    audit_id: 'sog_evidence_archive_review_history_audit_v3_pr387_2026_07_16',
    ...
    findings: [
      'Archive review history contains 40 reviewed events from PR #360, PR #365, PR #380, and PR #385 across 39 canonical Evidence identities.',
      'Twenty-six effective identities currently have recorded exact archives.',
      'Twelve reviewed unresolved identities remain suppressed: one invalid archive removal and eleven no-safe-change outcomes.',
      'The reviewed FDUSD source replacement remains archive-not-recorded and is eligible for one fresh manual archive review.',
      'Circle Mint is no longer reactivated because PR #385 added an exact dated archive.',
    ...
    decision: {
      ...
      archive_queue_generation_allowed_in_pr387: false,
      canonical_data_change_allowed: false,
      public_surface_change_allowed: false,
      review_gate_after_pr388: true
    },
    boundaries: {
...
export function writeEvidenceArchiveReviewHistoryV3Outputs(outputs = buildEvidenceArchiveReviewHistoryV3Outputs()) {
...
if (isCli) {
  ...
  } else writeEvidenceArchiveReviewHistoryV3Outputs(outputs);
