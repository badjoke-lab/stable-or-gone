...
const paths = {
  config: 'config/evidence-archive-maintenance-batch-7-pr400.json',
  decisions: 'config/evidence-archive-maintenance-batch-7-pr400-decisions.json',
  sourceQueue: 'docs/migration/evidence-archive-maintenance-queue-v6-pr398.json',
  probeQueue: 'docs/migration/evidence-archive-maintenance-batch-7-pr400-review-queue.json',
  authority: 'docs/migration/post-pr398-review-gate-pr399.json',
...
const canonicalCheckpointId = 'sog_evidence_archive_maintenance_batch_7_canonical_112_checkpoint_pr400_2026_07_16';
const statsCheckpointId = 'sog_evidence_archive_maintenance_batch_7_112_checkpoint_pr400_2026_07_16';
...
function readBaseText(file) {
  try {
    ...
    if (process.env.SOG_PR400_ALLOW_DISK_BASE === '1') return readText(file);
...
function assertAuthority(config, authority, queue) {
  ...
  if (decision?.pr !== 400 || decision?.decision !== 'approved_bounded_manual_review') {
    throw new Error('PR #399 does not authorize PR #400');
  }
  if (queue.queue_id !== 'sog_evidence_archive_maintenance_queue_v6_pr398') {
  ...
  if (JSON.stringify(selected) !== JSON.stringify(config.selected_evidence_ids)) {
    throw new Error('PR #400 selected IDs differ from Queue v6');
  }
  if (JSON.stringify(decision.selected_evidence_ids) !== JSON.stringify(config.selected_evidence_ids)) {
    throw new Error('PR #399 selected authority differs from PR #400 config');
  }
}
...
function buildCanonicalFiles(queue, decisions) {
  ...
  for (const file of sourceFiles) {
    ...
    outputs[file] = rows.map((row) => {
      ...
      if (row.url !== decision.canonical_url) throw new Error(`${row.id}: base URL changed before PR #400`);
...
function buildCheckpoint(base, decisions) {
  ...
  return {
    ...
    source_commit: 'pr400-evidence-archive-maintenance-batch-7',
    asset_count: 112,
...
function buildStatsCheckpoint(base, checkpoint) {
  return {
    ...
    checkpoint_kind: 'non_growth_normalization_checkpoint',
    recorded_at: '2026-07-16',
    registry_version: 'pr400-evidence-archive-maintenance-batch-7',
    asset_count: 112,
    source_checkpoint_id: base.checkpoint_id,
    canonical_checkpoint_id: canonicalCheckpointId,
    previous_history_checkpoint_id: base.checkpoint_id,
    maintenance_pr: 400,
    notes: `Reviewed forward same-count statistics checkpoint for PR #400 Evidence and Archive Maintenance Batch 7. It records archive coverage ${checkpoint.evidence_quality.archive_index_count} of 559 while preserving every canonical record count and public-surface boundary.`
  };
}
...
function buildReleaseBaseline(base, checkpoint, decisions) {
  return {
    ...base,
    status: 'current',
    baseline_id: 'sog_release_integrity_pr400_112_assets_2026_07_16',
    recorded_at: '2026-07-16',
    source_checkpoint_commit: 'pr400-evidence-archive-maintenance-batch-7',
    evidence_quality: {
...
function buildOutcomes(queue, decisions, checkpoint, canonicalFiles) {
  ...
  return {
    schema_version: '1.0',
    outcome_id: 'sog_evidence_archive_maintenance_outcomes_pr400_2026_07_16',
...
export function buildEvidenceArchiveMaintenanceBatch7Outputs() {
  ...
  const handoffBase = {
    schema_version: '1.0',
    handoff_id: 'sog_evidence_archive_maintenance_batch_7_pr400_reviewed_handoff_2026_07_16',
    status: 'reviewed_complete',
    review_pr: 400,
    source_pr: 399,
    source_merge_commit: '1e4bb90a52c787076c891372708898a81c98ec81',
    queue_id: queue.queue_id,
    ...
    next_work_item: {
      ...
      reason: 'Merged PR #399 authorizes only PR #400 before another review gate; no later archive, dossier, Market Access, growth, or public-surface work is pre-authorized.'
    },
    boundaries: {
      new_asset: false,
...
if (isCli) {
  ...
  if (process.argv.includes('--check')) {
    const stale = Object.entries(result.files)
      ...
      .map(([file]) => file);
    if (stale.length) {
      console.error(`PR #400 outputs are not reproducible: ${stale.join(', ')}`);
      process.exit(1);
    }
  }
