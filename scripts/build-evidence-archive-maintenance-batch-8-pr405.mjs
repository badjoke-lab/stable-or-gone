...
const paths = {
  config: 'config/evidence-archive-maintenance-batch-8-pr405.json',
  decisions: 'config/evidence-archive-maintenance-batch-8-pr405-decisions.json',
  sourceQueue: 'docs/migration/evidence-archive-maintenance-queue-v7-pr403.json',
  probeQueue: 'docs/migration/evidence-archive-maintenance-batch-8-pr405-review-queue.json',
  authority: 'docs/migration/post-pr403-review-gate-pr404.json',
...
const outputPaths = {
  ...
  outcomes: 'docs/migration/evidence-archive-maintenance-outcomes-pr405.json',
  handoff: 'docs/migration/evidence-archive-maintenance-batch-8-pr405-reviewed-handoff.json',
  checkpoint: paths.checkpoint,
  statsCheckpoint: paths.statsCheckpoint,
  statsHistory: paths.statsHistory,
  releaseBaseline: paths.releaseBaseline
};
const canonicalCheckpointId = 'sog_evidence_archive_maintenance_batch_8_canonical_112_checkpoint_pr405_2026_07_17';
const statsCheckpointId = 'sog_evidence_archive_maintenance_batch_8_112_checkpoint_pr405_2026_07_17';
...
function readBaseText(file) {
  try {
    ...
    if (process.env.SOG_PR405_ALLOW_DISK_BASE === '1') return readText(file);
...
function assertAuthority(config, authority, queue) {
  ...
  if (decision?.pr !== 405 || decision?.decision !== 'approved_bounded_manual_review') {
    throw new Error('PR #404 does not authorize PR #405');
  }
  if (queue.queue_id !== 'sog_evidence_archive_maintenance_queue_v7_pr403') {
  ...
  if (JSON.stringify(selected) !== JSON.stringify(config.selected_evidence_ids)) {
    throw new Error('PR #405 selected IDs differ from Queue v7');
  }
  if (JSON.stringify(decision.selected_evidence_ids) !== JSON.stringify(config.selected_evidence_ids)) {
    throw new Error('PR #404 selected authority differs from PR #405 config');
  }
}
...
function validateDecisions(config, decisionFile, probeQueue, queue) {
  ...
  if (decisions.length !== config.selected_evidence_ids.length) throw new Error('Decision count changed');
  if (JSON.stringify(decisions.map((row) => row.evidence_id)) !== JSON.stringify(config.selected_evidence_ids)) {
  ...
  for (const decision of decisions) {
    if (!allowed.has(decision.outcome)) throw new Error(`${decision.evidence_id}: invalid outcome`);
    ...
    if (decision.outcome === 'dated_exact_archive_added') {
      ...
      if (match.statuscode !== '200') throw new Error(`${decision.evidence_id}: capture is not HTTP 200`);
      ...
      if (decision.archived_url !== expectedArchive) throw new Error(`${decision.evidence_id}: archive URL mismatch`);
    }
    if (decision.outcome === 'reviewed_no_safe_change' && probe.exact_cdx_probe?.capture_count > 0) {
      throw new Error(`${decision.evidence_id}: no-safe-change conflicts with exact capture`);
    }
    if (decision.outcome === 'reviewed_source_replacement') {
      if (!decision.replacement_url || probe.live_probe?.final_url !== decision.replacement_url || !probe.live_probe?.redirected || !probe.live_probe?.ok) {
        throw new Error(`${decision.evidence_id}: replacement redirect not reviewed`);
      }
      if (decision.claim_scope_equivalence !== 'reviewed_equivalent' || !decision.source_version_equivalence?.startsWith('reviewed_')) {
        throw new Error(`${decision.evidence_id}: replacement equivalence incomplete`);
      }
    }
  }
}
...
function buildCanonicalFiles(queue, decisions) {
  ...
  for (const file of sourceFiles) {
    ...
    outputs[file] = rows.map((row) => {
      ...
      if (row.url !== decision.canonical_url) throw new Error(`${row.id}: base URL changed before PR #405`);
...
function buildCheckpoint(base, decisions) {
  ...
  return {
    ...
    source_commit: 'pr405-evidence-archive-maintenance-batch-8',
    asset_count: 112,
...
function buildStatsCheckpoint(base, checkpoint) {
  return {
    ...
    checkpoint_kind: 'non_growth_normalization_checkpoint',
    recorded_at: '2026-07-17',
    registry_version: 'pr405-evidence-archive-maintenance-batch-8',
    asset_count: 112,
    source_checkpoint_id: base.checkpoint_id,
    canonical_checkpoint_id: canonicalCheckpointId,
    previous_history_checkpoint_id: base.checkpoint_id,
    maintenance_pr: 405,
    notes: `Reviewed forward same-count statistics checkpoint for PR #405 Evidence and Archive Maintenance Batch 8. It records archive coverage ${checkpoint.evidence_quality.archive_index_count} of 559 while preserving every canonical record count and public-surface boundary.`
  };
}
...
function buildReleaseBaseline(base, checkpoint, decisions) {
  return {
    ...base,
    status: 'current',
    baseline_id: 'sog_release_integrity_pr405_112_assets_2026_07_17',
    recorded_at: '2026-07-17',
    source_checkpoint_commit: 'pr405-evidence-archive-maintenance-batch-8',
    evidence_quality: {
...
function buildOutcomes(queue, decisions, checkpoint, canonicalFiles) {
  ...
  return {
    schema_version: '1.0',
    outcome_id: 'sog_evidence_archive_maintenance_outcomes_pr405_2026_07_17',
    ...
    archive_index_count_before: 430,
...
export function buildEvidenceArchiveMaintenanceBatch8Outputs() {
  ...
  const handoffBase = {
    schema_version: '1.0',
    handoff_id: 'sog_evidence_archive_maintenance_batch_8_pr405_reviewed_handoff_2026_07_17',
    status: 'reviewed_complete',
    review_pr: 405,
    source_pr: 404,
    source_merge_commit: 'bd52135d764a8e91ad9055b215c887beed227c53',
    queue_id: queue.queue_id,
    ...
    next_work_item: {
      ...
      reason: 'Merged PR #404 authorizes only PR #405 before another review gate; no later archive, dossier, Market Access, growth, editorial, monitoring-registration, or public-surface work is pre-authorized.'
    },
    boundaries: {
      new_asset: false,
...
if (isCli) {
  ...
  if (process.argv.includes('--check')) {
    const stale = Object.entries(result.files)
      ...
      .map(([file]) => file);
    if (stale.length) {
      console.error(`PR #405 outputs are not reproducible: ${stale.join(', ')}`);
      process.exit(1);
    }
  }
