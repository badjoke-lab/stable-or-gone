...
const queuePath = 'docs/migration/evidence-archive-maintenance-queue-v6-pr398.json';
const configPath = 'config/evidence-archive-maintenance-batch-7-pr400.json';
const authorityPath = 'docs/migration/post-pr398-review-gate-pr399.json';
const outputPath = 'docs/migration/evidence-archive-maintenance-batch-7-pr400-review-queue.json';
...
async function probeCdx(url) {
  ...
  try {
    ...
    if (!response.ok) return { ok: false, status: response.status, query_url: query.toString(), captures: [], error: body.slice(0, 500) };
    ...
    return { ok: false, status: null, query_url: query.toString(), captures: [], error: error instanceof Error ? error.message : String(error) };
  }
}
...
if (decision?.pr !== 400 || decision?.decision !== 'approved_bounded_manual_review') throw new Error('PR #399 does not authorize PR #400');
const selected = queue.selected_candidates;
const selectedIds = selected.map((row) => row.evidence_id);
if (JSON.stringify(selectedIds) !== JSON.stringify(config.selected_evidence_ids)) throw new Error('PR #400 config selected IDs do not match Queue v6 order');
if (JSON.stringify(decision.selected_evidence_ids) !== JSON.stringify(config.selected_evidence_ids)) throw new Error('PR #399 selected authority differs from PR #400 config');
...
const output = {
  schema_version: '1.0',
  queue_id: 'sog_evidence_archive_maintenance_batch_7_pr400_review_queue',
