...
for (const row of rows) {
  ...
  const value = row.canonicality ?? 'unknown';
...
const view = foundation.derived_views?.chain_canonicality;
const expected = baseline.minimum_counts.deployments;
if (!view) failures.push('missing chain_canonicality view');
else {
  if (view.source_group !== 'deployments') failures.push('invalid source_group');
