...
assert(audit.totals?.legacy_update_entries === 15, `expected 15 legacy update entries, found ${audit.totals?.legacy_update_entries}`);
...
assert(audit.totals?.legacy_public_copy_overrides === 15, 'all 15 legacy updates must retain public-copy overlays');
...
assert(JSON.stringify(audit.legacy_updates?.categories) === JSON.stringify({ content: 3, data: 9, foundation: 1, ui: 2 }), 'legacy category inventory changed unexpectedly');
