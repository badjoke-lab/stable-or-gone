...
const paths = {
  config: 'config/evidence-archive-maintenance-queue-v3-pr383.json',
  ...
  priorQueue: 'docs/migration/evidence-archive-maintenance-queue-v2-pr378.json'
};
const outputPaths = {
  queue: 'docs/migration/evidence-archive-maintenance-queue-v3-pr383.json',
  delta: 'docs/migration/evidence-archive-maintenance-queue-v3-pr383-delta.json'
};
...
export function buildEvidenceArchiveMaintenanceQueueV3Outputs() {
  ...
  if (authority.decisions?.evidence_archive_maintenance_queue_v3?.pr !== 383) {
    throw new Error('PR #381 does not authorize PR #383');
  }
  if (historyAudit.decision?.next_work_item !== 'PR #383 Evidence Archive Maintenance Queue v3 Refresh') {
    throw new Error('PR #382 audit does not hand off to PR #383');
  }
  if (contract.contract_id !== 'sog_evidence_archive_review_history_v2_pr382') {
    throw new Error(`Unexpected archive review-history contract ${contract.contract_id}`);
  }
  if (historyManifest.manifest_id !== 'sog_evidence_archive_review_history_manifest_v2_pr382') {
    throw new Error(`Unexpected archive review-history manifest ${historyManifest.manifest_id}`);
  }
  if (historyAudit.audit_id !== 'sog_evidence_archive_review_history_audit_v2_pr382_2026_07_16') {
    throw new Error(`Unexpected archive review-history audit ${historyAudit.audit_id}`);
  }
  ...
  const selectedCandidates = selected.map(({ row, priority, history, reactivated, selectionTier }) => ({
    ...
    review_reasons: [
      ...
      reactivated ? 'reviewed_reactivated_history_v2' : 'unreviewed_archive_gap',
      priority.bucket
    ],
  ...
  const selectedReactivatedIds = selectedCandidates
    .filter((row) => row.reactivation_signal_present)
    .map((row) => row.evidence_id)
    .sort();

  const queue = {
    schema_version: '3.0',
    queue_id: 'sog_evidence_archive_maintenance_queue_v3_pr383',
    status: 'reviewed_internal_non_ranking_history_v2_queue',
    public_output: false,
    evidence_rank: false,
  ...
  const delta = {
    schema_version: '2.0',
    delta_id: 'sog_evidence_archive_maintenance_queue_v3_delta_pr383',
    status: 'reviewed_internal_archive_queue_history_v2_delta',
    public_output: false,
    review_pr: 383,
    reviewed_at: config.reviewed_at,
    prior_queue_id: priorQueue.queue_id,
...
export function writeEvidenceArchiveMaintenanceQueueV3Outputs(outputs = buildEvidenceArchiveMaintenanceQueueV3Outputs()) {
...
if (isCli) {
  const outputs = buildEvidenceArchiveMaintenanceQueueV3Outputs();
  ...
  } else writeEvidenceArchiveMaintenanceQueueV3Outputs(outputs);
