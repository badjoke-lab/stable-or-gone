...
const paths = {
  config: 'config/post-pr385-review-gate-pr386.json',
  ...
  historyContract: 'config/evidence-archive-review-history-v2-pr382.json',
  historyManifest: 'docs/migration/evidence-archive-review-history-manifest-v2-pr382.json',
  historyAudit: 'docs/migration/evidence-archive-review-history-audit-v2-pr382.json',
...
const outputPath = 'docs/migration/post-pr385-review-gate-pr386.json';
...
function mapPr385Outcome(row) {
  ...
  throw new Error(`${row.evidence_id}: unsupported PR #385 outcome ${row.decision}`);
}

function projectHistoryV3(manifest, outcomes) {
  ...
  for (const row of outcomes.outcomes) {
    effective.set(row.evidence_id, {
      evidence_id: row.evidence_id,
      effective_review_outcome: mapPr385Outcome(row),
      effective_review_pr: 385,
...
export function buildPostPr385ReviewGate() {
  ...
  if (handoff.next_work_item?.decision !== 'review_gate_required') throw new Error('PR #385 handoff does not require PR #386');
  if (outcomes.outcome_id !== handoff.outcome_id) throw new Error('PR #385 outcome/handoff identity mismatch');
  const projection = projectHistoryV3(historyManifest, outcomes);
  ...
  return {
    schema_version: '1.0',
    report_id: 'sog_post_pr385_review_gate_pr386_2026_07_16',
    ...
    current_history_v2: {
      ...
      latest_included_review_pr: 380,
      stale_after_pr385: true
    },
    expected_history_v3: projection,
    findings: [
      'PR #385 completed a bounded ten-identity review with seven exact archive additions, one reviewed source replacement, and two reviewed no-safe-change outcomes.',
      'Archive coverage advanced from 399/160 to 406/153 while Evidence identities and Evidence Relations remained 559/559.',
      'History v2 is stale because it excludes all PR #385 review events.',
      'Circle Mint resolves to archive-present under History v3; the FDUSD product route becomes the sole reviewed source-replacement eligibility.',
      'Queue v3 is consumed and may not authorize another canonical batch.'
    ],
    decisions: {
      evidence_archive_review_history_contract_v3: {
      ...
      evidence_archive_maintenance_queue_v4: {
        decision: 'approved_internal_after_pr387',
        pr: 388,
      ...
      evidence_archive_maintenance_batch_5: { decision: 'not_approved' },
    ...
    activation_rule: 'PR #387 must update AGENTS.md and docs/roadmap.md before generating History v3 outputs.',
    boundaries: {
...
export function writePostPr385ReviewGate(report = buildPostPr385ReviewGate()) {
...
if (isCli) {
  const report = buildPostPr385ReviewGate();
  ...
  } else writePostPr385ReviewGate(report);
  console.log(JSON.stringify({
    ...
    pr385_yield: `${report.completed_pr385.changed}/${report.completed_pr385.selected}`,
    history_v3: {
      sources: report.expected_history_v3.sources,
      events: report.expected_history_v3.events,
      identities: report.expected_history_v3.identities,
      archive_present: report.expected_history_v3.effective_archive_present,
      invalid_removed: report.expected_history_v3.effective_invalid_removed,
      no_safe_change: report.expected_history_v3.effective_no_safe_change,
      source_replacement: report.expected_history_v3.effective_source_replacement,
      unresolved_total: report.expected_history_v3.reviewed_unresolved_total,
      suppressed: report.expected_history_v3.reviewed_suppressed,
      reactivated: report.expected_history_v3.reviewed_reactivated_eligible
    },
