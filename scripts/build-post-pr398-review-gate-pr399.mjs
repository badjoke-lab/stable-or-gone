...
const paths = {
  config: 'config/post-pr398-review-gate-pr399.json',
...
const outputPath = 'docs/migration/post-pr398-review-gate-pr399.json';
const readText = (file) => fs.readFileSync(path.join(root, file), 'utf8');
const readJson = (file) => JSON.parse(readText(file));
const serialize = (value) => `${JSON.stringify(value, null, 2)}\n`;
const sha256 = (value) => crypto.createHash('sha256').update(value).digest('hex');

export function buildPostPr398ReviewGate() {
  ...
  const actual = {
    ...
    added_vs_queue_v5: delta.added_evidence_ids.length,
    removed_vs_queue_v5: delta.removed_evidence_ids.length,
    retained_vs_queue_v5: delta.retained_evidence_ids.length,
    selected_evidence_ids: selectedIds
  };
  if (JSON.stringify(actual) !== JSON.stringify(expected)) throw new Error(`PR #399 input differs from contract: ${JSON.stringify(actual)}`);
  if (queue.next_work_item !== 'REVIEW GATE' || queue.selection_boundary.batch_7_authorized !== false) throw new Error('Queue v6 does not stop at review gate');
  if (queue.selected_candidates.some((row) => row.review_history_found || row.selection_tier !== 1 || row.canonical_change_authorized)) throw new Error('Queue v6 candidate boundary changed');

  const sourceDigest = sha256(Object.values(paths).map((file) => `${file}\0${readText(file)}`).join('\0'));
  const reportBase = {
    schema_version: '1.0',
    report_id: 'sog_post_pr398_review_gate_pr399_2026_07_16',
    ...
    queue_review: {
      ...
      selected_priority_buckets: [...new Set(queue.selected_candidates.map((row) => row.priority_bucket))].sort(),
      ...
      result: 'bounded_fresh_history_v5_queue'
    },
    decisions: {
      evidence_archive_maintenance_batch_7: {
    ...
    activation_rule: 'PR #400 must update AGENTS.md and docs/roadmap.md before changing canonical Evidence.',
    review_gate_after_sequence: true,
    findings: [
      'Queue v6 is deterministic, non-ranking, and contains ten unique ordinary unreviewed archive gaps.',
      'History v5 suppresses thirteen previously reviewed unresolved identities and exposes no reviewed-reactivated candidate.',
      'The ten Queue v5 identities were removed because PR #395 either recorded exact archives or established reviewed no-safe-change suppression.',
      'Queue v6 selects one Nuon source, eight Paxos or PayPal product and support sources, and one Reflexer source for bounded manual source and archive review.',
      'No Queue v6 candidate is presumed to change and Queue v6 itself authorizes no canonical update.'
    ],
    boundaries: {
...
export function writePostPr398ReviewGate(report = buildPostPr398ReviewGate()) {
...
if (isCli) {
  const report = buildPostPr398ReviewGate();
  ...
  } else writePostPr398ReviewGate(report);
  console.log(JSON.stringify({
    ...
    batch_7_decision: report.decisions.evidence_archive_maintenance_batch_7.decision,
