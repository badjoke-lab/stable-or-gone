...
const paths = {
  config: 'config/evidence-archive-maintenance-queue-v6-pr398.json',
  ...
  priorQueue: 'docs/migration/evidence-archive-maintenance-queue-v5-pr393.json'
};
const outputPaths = {
  queue: 'docs/migration/evidence-archive-maintenance-queue-v6-pr398.json',
  delta: 'docs/migration/evidence-archive-maintenance-queue-v6-pr398-delta.json'
};
...
export function buildEvidenceArchiveMaintenanceQueueV6Outputs() {
  ...
  if (authority.decisions?.evidence_archive_maintenance_queue_v6?.pr !== 398) throw new Error('PR #396 does not authorize PR #398');
  if (authority.decisions?.evidence_archive_maintenance_queue_v6?.decision !== 'approved_internal_after_pr397') throw new Error('PR #396 Queue v6 decision changed');
  if (historyAudit.decision?.next_work_item !== 'PR #398 Evidence Archive Maintenance Queue v6 Refresh') throw new Error('PR #397 audit does not hand off to PR #398');
  if (contract.contract_id !== 'sog_evidence_archive_review_history_v5_pr397') throw new Error(`Unexpected contract ${contract.contract_id}`);
  if (historyManifest.manifest_id !== 'sog_evidence_archive_review_history_manifest_v5_pr397') throw new Error(`Unexpected manifest ${historyManifest.manifest_id}`);
  if (historyAudit.audit_id !== 'sog_evidence_archive_review_history_audit_v5_pr397_2026_07_16') throw new Error(`Unexpected audit ${historyAudit.audit_id}`);
  ...
  if (historyManifest.counts.history_source_count !== input.history_source_count || historyManifest.counts.history_event_count !== input.history_event_count) throw new Error('History v5 source/event count changed');
  if (historyManifest.counts.reviewed_evidence_identity_count !== input.reviewed_identity_count) throw new Error('History v5 identity count changed');
  if (historyAudit.reviewed_unresolved.suppressed_count !== input.reviewed_suppressed_count) throw new Error('History v5 suppression count changed');
  if (historyAudit.reviewed_unresolved.reactivated_eligible_count !== input.reviewed_reactivated_eligible_count) throw new Error('History v5 reactivation count changed');
  ...
  const selectedCandidates = selected.map(({ row, priority, history, reactivated, selectionTier }) => ({
    ...
    review_reasons: ['archive_not_recorded', reactivated ? 'reviewed_reactivated_history_v5' : 'unreviewed_archive_gap', priority.bucket],
  ...
  const queue = {
    schema_version: '6.0',
    queue_id: 'sog_evidence_archive_maintenance_queue_v6_pr398',
    status: 'reviewed_internal_non_ranking_history_v5_queue',
    public_output: false,
    evidence_rank: false,
  ...
  const delta = {
    schema_version: '5.0',
    delta_id: 'sog_evidence_archive_maintenance_queue_v6_delta_pr398',
    status: 'reviewed_internal_archive_queue_history_v5_delta',
    public_output: false,
    review_pr: 398,
    reviewed_at: config.reviewed_at,
    prior_queue_id: priorQueue.queue_id,
...
export function writeEvidenceArchiveMaintenanceQueueV6Outputs(outputs = buildEvidenceArchiveMaintenanceQueueV6Outputs()) {
...
if (isCli) {
  ...
  if (process.argv.includes('--check')) {
    ...
    writeEvidenceArchiveMaintenanceQueueV6Outputs(outputs);
  }
