...
if (report.audited_counts?.stable_assets !== 100) failures.push(`expected 100 stable assets, got ${report.audited_counts?.stable_assets}`);
if (report.audited_counts?.promoted_candidate_mappings !== 100) failures.push(`expected 100 promoted candidate mappings, got ${report.audited_counts?.promoted_candidate_mappings}`);
...
console.log(`Identity and lineage audit validation passed: ${report.audited_counts.stable_assets} assets, 0 critical findings, 0 alias gaps, 0 lineage cycles.`);
