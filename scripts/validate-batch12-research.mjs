...
const promotions = JSON.parse(fs.readFileSync('data/candidate-promotions-batch-l.json', 'utf8'));
const errors = [];
...
check(research.policy?.canonical_write_allowed === false, 'research contract cannot directly write canonical data');
...
const promotedIds = new Set(promotions.filter((item) => item.status === 'promoted').map((item) => item.candidate_id));
...
for (const record of research.records ?? []) {
  ...
  check(promotedIds.has(record.candidate_id), `promotion missing: ${record.candidate_id}`);
  ...
  else check(record.launch_precision !== 'day', `null launch date cannot claim day precision: ${record.candidate_id}`);
  ...
  check(record.promotion_readiness === 'promoted', `research record must be promoted: ${record.candidate_id}`);
}
...
if (errors.length) { for (const error of errors) console.error(`Batch 12 research: ${error}`); process.exitCode = 1; }
else console.log('Batch 12 research contract valid: 5 reviewed candidates, 5 promotions');
