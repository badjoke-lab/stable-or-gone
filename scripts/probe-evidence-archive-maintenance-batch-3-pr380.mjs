...
const queuePath = 'docs/migration/evidence-archive-maintenance-queue-v2-pr378.json';
const configPath = 'config/evidence-archive-maintenance-batch-3-pr380.json';
const outputPath = 'docs/migration/evidence-archive-maintenance-batch-3-pr380-review-queue.json';
...
async function probeCdx(url) {
  ...
  try {
    ...
    if (!response.ok) {
      return { ok: false, status: response.status, query_url: query.toString(), captures: [], error: body.slice(0, 500) };
    }
    ...
    const sampled = captures.length <= 16
      ? captures
      : [...captures.slice(0, 8), ...captures.slice(-8)];
    return {
      ok: true,
      status: response.status,
...
if (JSON.stringify(selectedIds) !== JSON.stringify(config.selected_evidence_ids)) {
  throw new Error('PR #380 config selected IDs do not match PR #378 queue order');
}
...
const output = {
  schema_version: '1.0',
  queue_id: 'sog_evidence_archive_maintenance_batch_3_pr380_review_queue',
