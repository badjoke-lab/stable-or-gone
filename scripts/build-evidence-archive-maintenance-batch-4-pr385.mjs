...
const paths = {
  config: 'config/evidence-archive-maintenance-batch-4-pr385.json',
  decisions: 'config/evidence-archive-maintenance-batch-4-pr385-decisions.json',
  sourceQueue: 'docs/migration/evidence-archive-maintenance-queue-v3-pr383.json',
  probeQueue: 'docs/migration/evidence-archive-maintenance-batch-4-pr385-review-queue.json',
  authority: 'docs/migration/post-pr383-review-gate-pr384.json',
...
const outputPaths = {
  ...
  outcomes: 'docs/migration/evidence-archive-maintenance-outcomes-pr385.json',
  handoff: 'docs/migration/evidence-archive-maintenance-batch-4-pr385-reviewed-handoff.json',
  checkpoint: paths.checkpoint,
  statsCheckpoint: paths.statsCheckpoint,
  statsHistory: paths.statsHistory,
  releaseBaseline: paths.releaseBaseline
};
const canonicalCheckpointId = 'sog_evidence_archive_maintenance_batch_4_canonical_112_checkpoint_pr385_2026_07_16';
const statsCheckpointId = 'sog_evidence_archive_maintenance_batch_4_112_checkpoint_pr385_2026_07_16';
...
function readBaseText(file) {
  try {
    ...
    if (process.env.SOG_PR385_ALLOW_DISK_BASE === '1') return readText(file);
...
function equivalentExactUrl(left, right) {
  try {
    return decodeURIComponent(String(left)) === decodeURIComponent(String(right));
  } catch {
...
function assertAuthority(config, authority, queue) {
  const decision = authority.decisions?.evidence_archive_maintenance_batch_4;
  if (decision?.pr !== 385 || decision?.decision !== 'approved_bounded') throw new Error('PR #384 does not authorize PR #385');
  if (queue.queue_id !== 'sog_evidence_archive_maintenance_queue_v3_pr383') throw new Error(`Unexpected source queue ${queue.queue_id}`);
  const selected = queue.selected_candidates.map((row) => row.evidence_id);
  if (JSON.stringify(selected) !== JSON.stringify(config.selected_evidence_ids)) throw new Error('PR #385 selected IDs differ from PR #383 queue');
  if (JSON.stringify(decision.selected_evidence_ids) !== JSON.stringify(config.selected_evidence_ids)) throw new Error('PR #384 selected authority differs from PR #385 config');
}
...
function validateDecisions(config, decisionFile, probeQueue, queue) {
  ...
  for (const decision of decisions) {
    ...
    if (decision.outcome === 'dated_exact_archive_added') {
      ...
      if (!equivalentExactUrl(match.original, decision.canonical_url)) throw new Error(`${decision.evidence_id}: accepted capture is not the exact canonical source identity`);
...
function buildCanonicalFiles(queue, decisions) {
  ...
  for (const file of sourceFiles) {
    ...
    outputs[file] = rows.map((row) => {
      ...
      if (row.url !== decision.canonical_url) throw new Error(`${row.id}: base canonical URL changed before PR #385`);
...
function buildCheckpoint(baseCheckpoint, decisions) {
  ...
  const archiveAfter = baseCheckpoint.evidence_quality.archive_index_count + archiveAdded.length;
  const missingAfter = baseCheckpoint.evidence_quality.archive_not_recorded_count - archiveAdded.length;
  return {
    schema_version: '1.0',
    ...
    source_commit: 'pr385-evidence-archive-maintenance-batch-4',
    asset_count: 112,
    ...
    notes: `Current deterministic canonical checkpoint after PR #385 Evidence and Archive Maintenance Batch 4. ${archiveAdded.length} exact dated archive captures and ${replacements.length} reviewed source replacement were accepted; ${noSafe.length} identities recorded reviewed no-safe-change. Archive coverage is ${archiveAfter} of 559. All canonical identity counts, Evidence Relations, assets, Market Access records, non-Evidence record families, and public surfaces remain unchanged.`
  };
}

function buildStatsCheckpoint(baseStatsCheckpoint, checkpoint) {
  return {
    schema_version: '1.0',
    status: 'reviewed_non_growth_checkpoint',
    ...
    checkpoint_kind: 'non_growth_normalization_checkpoint',
    recorded_at: '2026-07-16',
    registry_version: 'pr385-evidence-archive-maintenance-batch-4',
    asset_count: 112,
    ...
    maintenance_pr: 385,
    notes: `Reviewed forward same-count statistics checkpoint for PR #385 Evidence and Archive Maintenance Batch 4. It records archive coverage ${checkpoint.evidence_quality.archive_index_count} of 559 while preserving every canonical record count and public-surface boundary.`
  };
}

function buildReleaseBaseline(baseBaseline, checkpoint, decisions) {
  return {
    ...baseBaseline,
    status: 'current',
    baseline_id: 'sog_release_integrity_pr385_112_assets_2026_07_16',
    recorded_at: '2026-07-16',
    source_checkpoint_commit: 'pr385-evidence-archive-maintenance-batch-4',
    evidence_quality: {
...
function buildOutcomes(queue, decisions, checkpoint, canonicalFiles) {
  ...
  return {
    schema_version: '1.0',
    outcome_id: 'sog_evidence_archive_maintenance_outcomes_pr385_2026_07_16',
...
export function buildEvidenceArchiveMaintenanceBatch4Outputs() {
  ...
  const handoffBase = {
    schema_version: '1.0',
    handoff_id: 'sog_evidence_archive_maintenance_batch_4_pr385_reviewed_handoff_2026_07_16',
    status: 'reviewed_complete',
    review_pr: 385,
    source_pr: 384,
    source_merge_commit: 'aecf80517a07c23c0d27e986031346756dfee44a',
    queue_id: queue.queue_id,
    ...
    next_work_item: {
      ...
      reason: 'Merged PR #384 authorizes only PR #385 before another review gate; no later archive, dossier, Market Access, growth, or public-surface work is pre-authorized.'
    },
    boundaries: {
      new_asset: false,
...
if (isCli) {
  ...
  if (process.argv.includes('--check')) {
    ...
    if (stale.length) {
      console.error(`PR #385 outputs are not reproducible: ${stale.join(', ')}`);
      process.exit(1);
    }
  }
