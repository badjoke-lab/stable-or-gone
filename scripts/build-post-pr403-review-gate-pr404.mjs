...
const paths = {
  config: 'config/post-pr403-review-gate-pr404.json',
  ...
  historyContract: 'config/evidence-archive-review-history-v6-pr402.json',
...
const outputPath = 'docs/migration/post-pr403-review-gate-pr404.json';
const readText = (file) => fs.readFileSync(path.join(root, file), 'utf8');
const readJson = (file) => JSON.parse(readText(file));
const serialize = (value) => `${JSON.stringify(value, null, 2)}\n`;
const sha256 = (value) => crypto.createHash('sha256').update(value).digest('hex');

export function buildPostPr403ReviewGate() {
  ...
  const actual = {
    ...
    added_vs_queue_v6: delta.added_evidence_ids.length,
    removed_vs_queue_v6: delta.removed_evidence_ids.length,
    retained_vs_queue_v6: delta.retained_evidence_ids.length,
    selected_evidence_ids: selectedIds
  };
  if (JSON.stringify(actual) !== JSON.stringify(expected)) throw new Error(`PR #404 input differs from contract: ${JSON.stringify(actual)}`);
  if (queue.next_work_item !== 'REVIEW GATE' || queue.selection_boundary.batch_8_authorized !== false) throw new Error('Queue v7 does not stop at review gate');
  if (queue.selected_candidates.some((row) => row.review_history_found || row.selection_tier !== 1 || row.canonical_change_authorized)) throw new Error('Queue v7 candidate boundary changed');

  const sourceDigest = sha256(Object.values(paths).map((file) => `${file}\0${readText(file)}`).join('\0'));
  const reportBase = {
    schema_version: '1.0',
    report_id: config.generated_report_id,
    ...
    queue_review: {
      ...
      result: 'bounded_fresh_history_v6_queue'
    },
    decisions: {
      evidence_archive_maintenance_batch_8: {
      ...
      evidence_archive_maintenance_batch_9: { decision: 'not_approved' },
    ...
    activation_rule: 'PR #405 must update AGENTS.md and docs/roadmap.md before changing canonical Evidence.',
    review_gate_after_sequence: true,
    findings: [
      'Queue v7 is deterministic, non-ranking, and contains ten unique ordinary unreviewed archive gaps.',
      'History v6 suppresses eighteen previously reviewed unresolved identities and exposes no reviewed-reactivated candidate.',
      'Queue v7 selects three Reflexer RAI sources, three Ripple RLUSD sources, and four Ampleforth SPOT sources.',
      'No Queue v7 candidate is presumed to change and Queue v7 itself authorizes no canonical update.'
    ],
    boundaries: {
...
export function writePostPr403ReviewGate(report = buildPostPr403ReviewGate()) {
...
if (isCli) {
  const report = buildPostPr403ReviewGate();
  ...
  } else writePostPr403ReviewGate(report);
  console.log(JSON.stringify({
    ...
    batch_8_decision: report.decisions.evidence_archive_maintenance_batch_8.decision,
