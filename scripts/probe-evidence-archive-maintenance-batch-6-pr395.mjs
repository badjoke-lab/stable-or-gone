...
const queuePath = 'docs/migration/evidence-archive-maintenance-queue-v5-pr393.json';
const configPath = 'config/evidence-archive-maintenance-batch-6-pr395.json';
const authorityPath = 'docs/migration/post-pr393-review-gate-pr394.json';
const outputPath = 'docs/migration/evidence-archive-maintenance-batch-6-pr395-review-queue.json';
...
if (decision?.pr !== 395 || decision?.decision !== 'approved_bounded_manual_review') throw new Error('PR #394 does not authorize PR #395');
const selected = queue.selected_candidates;
const selectedIds = selected.map((row) => row.evidence_id);
if (JSON.stringify(selectedIds) !== JSON.stringify(config.selected_evidence_ids)) throw new Error('PR #395 config selected IDs do not match Queue v5 order');
if (JSON.stringify(decision.selected_evidence_ids) !== JSON.stringify(config.selected_evidence_ids)) throw new Error('PR #394 selected authority differs from PR #395 config');
...
const output = {
  schema_version: '1.0',
  queue_id: 'sog_evidence_archive_maintenance_batch_6_pr395_review_queue',
