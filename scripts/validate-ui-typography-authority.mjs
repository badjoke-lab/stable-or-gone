...
const technicalSelector = /(^|[\s,.>+~:[\]-])(code|pre|kbd|samp)(\b|[\s,.>+~:[\]-])|contract-address|transaction-hash|data-long-value|address-block\s+code|wallet-row\s+code|stablecoin-record-id/i;
...
for (const absolute of files) {
  ...
  if (!protectedFlatFiles.has(relative)) continue;
...
const result = {
  ...
  protected_source_files: [...protectedFlatFiles],
  computed_style_audit: 'scripts/audit-ui-remediation-r7.mjs',
  failures
};
