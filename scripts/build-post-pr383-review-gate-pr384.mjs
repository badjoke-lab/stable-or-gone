...
const paths = {
  config: 'config/post-pr383-review-gate-pr384.json',
  queue: 'docs/migration/evidence-archive-maintenance-queue-v3-pr383.json',
  queueDelta: 'docs/migration/evidence-archive-maintenance-queue-v3-pr383-delta.json',
...
const outputPath = 'docs/migration/post-pr383-review-gate-pr384.json';
const readText = (file) => fs.readFileSync(path.join(root, file), 'utf8');
const readJson = (file) => JSON.parse(readText(file));
const serialize = (value) => `${JSON.stringify(value, null, 2)}\n`;
const sha256 = (value) => crypto.createHash('sha256').update(value).digest('hex');

export function buildPostPr383ReviewGate() {
  ...
  return {
    schema_version: '1.0',
    report_id: 'sog_post_pr383_review_gate_pr384_2026_07_16',
    ...
    source_checkpoint: {
      ...
      evidence_relations: checkpoint.expected_counts.evidence_relations,
    ...
    completed_sequence: [
      { pr: 382, work_item: 'Evidence Archive Review-History Contract v2 Update', status: 'complete' },
      { pr: 383, work_item: 'Evidence Archive Maintenance Queue v3 Refresh', status: 'complete' }
    ],
    evaluation: {
      queue_integrity: {
        ...
        added_vs_queue_v2: delta.added_evidence_ids.length,
        removed_vs_queue_v2: delta.removed_evidence_ids.length,
        retained_vs_queue_v2: delta.retained_evidence_ids.length,
        ...
        result: 'bounded_fresh_history_v2_queue'
      },
      history_enforcement: {
        ...
        reviewed_reactivated_selected: audit.reviewed_unresolved.reactivated_eligible_evidence_ids.every((id) => selectedEvidenceIds.includes(id)),
      ...
      prior_batch_yield: {
        ...
        pr380: {
          selected: pr380.selected_count,
          changed: pr380.changed_count,
          dated_archive_added: pr380.dated_archive_added_count,
          reviewed_source_replacement: pr380.reviewed_source_replacement_count,
          no_safe_change: pr380.reviewed_no_safe_change_count,
          change_yield_percent: Number(((pr380.changed_count / pr380.selected_count) * 100).toFixed(2))
        },
        finding: 'Archive-maintenance yield varies by source set. PR #385 must review every Queue v3 identity and may not presume a canonical change.'
      },
      batch_4_readiness: {
    ...
    decisions: {
      evidence_archive_maintenance_batch_4: {
    ...
    activation_rule: 'PR #385 must update AGENTS.md and docs/roadmap.md before changing canonical Evidence.',
    boundaries: {
...
export function writePostPr383ReviewGate(report = buildPostPr383ReviewGate()) {
...
if (isCli) {
  const report = buildPostPr383ReviewGate();
  ...
  } else writePostPr383ReviewGate(report);
  console.log(JSON.stringify({
    ...
    batch_4_decision: report.decisions.evidence_archive_maintenance_batch_4.decision,
