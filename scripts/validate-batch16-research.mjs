...
const research = JSON.parse(fs.readFileSync('data/candidate-research-batch-16.json', 'utf8'));
const candidates = JSON.parse(fs.readFileSync('data/candidate-stable-assets-growth-85.json', 'utf8'));
const errors = [];
const check = (condition, message) => { if (!condition) errors.push(message); };

check(research.schema_version === '1.0', 'schema_version must be 1.0');
check(research.batch_id === 'batch_016', 'batch_id must be batch_016');
check(research.status === 'accepted', 'research status must be accepted');
check(research.canonical_assets === 82, 'canonical asset baseline must be 82');
...
for (const record of research.records ?? []) {
  ...
  if (candidate) {
    ...
    check(candidate.target_batch === 'batch_016', `candidate target batch mismatch: ${record.candidate_id}`);
  }
  ...
  check(record.proposed_status === 'active', `candidate must be active proposal: ${record.candidate_id}`);
...
const solstice = candidates.find((row) => row.proposed_record_id === 'sog_st_solsticeusx');
check(solstice?.slug === 'solstice-usx', 'Solstice USX must use disambiguated slug');
check(solstice?.proposed_record_id !== 'sog_st_usx', 'Solstice USX must not reuse dForce USX record id');

if (errors.length) {
  for (const error of errors) console.error(`Batch 16 research: ${error}`);
  process.exitCode = 1;
} else {
  console.log('Batch 16 research contract valid: 5 accepted candidates, zero canonical promotions');
}
