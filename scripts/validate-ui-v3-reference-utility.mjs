...
const pages = {
  ...
  updates: read('src/pages/updates/index.astro'),
...
for (const name of ['models', 'glossary', 'updates']) check(pages[name].includes('data-ui-v3-reference'), `${name}: reference marker missing`);
...
check(pages.updates.includes('getRegistryUpdates') && pages.updates.includes('reference-mobile-records'), 'Updates register or mobile records missing');
...
const result = { schema_version: '1.0', ok: failures.length === 0, reference_pages: 3, longform_pages: 2, utility_pages: 2, canonical_record_changes: 0, route_changes: 0, failures };
