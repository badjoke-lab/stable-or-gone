...
const canonicalUnresolvedIds = stablecoins
  .filter((row) => terminalStatuses.has(row.status) && row.discontinued_date === null)
  .map((row) => row.id)
  .sort();
...
const requiredPolicyFlags = [
  'canonical_day_required',
  'keep_null_without_matching_terminal_boundary_evidence',
  'forbid_depeg_as_default_terminal_date',
  'forbid_last_commit_as_shutdown_date',
  'forbid_migration_start_as_default_final_end',
  'forbid_retrospective_source_date_as_effective_date'
];
...
if (!process.exitCode) {
  console.log(`terminal-date unresolved queue valid: ${records.length} records (${queueIds.join(', ')})`);
}
