...
const paths = {
  config: 'config/post-pr395-review-gate-pr396.json',
...
const outputPath = 'docs/migration/post-pr395-review-gate-pr396.json';
...
function mapOutcome(row) {
  ...
  throw new Error(`${row.evidence_id}: unsupported PR #395 outcome ${row.decision}`);
}

function projectHistoryV5(manifest, outcomes) {
...
export function buildPostPr395ReviewGate() {
  ...
  if (handoff.next_work_item?.decision !== 'review_gate_required') throw new Error('PR #395 handoff does not require PR #396');
  if (outcomes.outcome_id !== handoff.outcome_id) throw new Error('PR #395 outcome/handoff mismatch');
  const projection = projectHistoryV5(historyManifest, outcomes);
  const expected = config.expected_history_v5_projection;
  const actual = {
  ...
  if (JSON.stringify(actual) !== JSON.stringify(expected)) throw new Error(`History v5 projection differs from contract: ${JSON.stringify(actual)}`);

  const sourceDigest = sha256(Object.values(paths).map((file) => `${file}\0${readText(file)}`).join('\0'));
  const reportBase = {
    schema_version: '1.0',
    report_id: 'sog_post_pr395_review_gate_pr396_2026_07_16',
    ...
    current_history_v4: {
      ...
      latest_included_review_pr: 390,
      stale_after_pr395: true
    },
    expected_history_v5: projection,
    findings: [
      'PR #395 completed ten bounded reviews with nine exact dated archive additions and one reviewed no-safe-change.',
      'Archive coverage advanced from 416/143 to 425/134 while Evidence identities and Relations remained 559/559.',
      'History v4 is stale because it excludes all PR #395 review events.',
      'The Sky documentation root becomes a new reviewed no-safe-change suppression.',
      'No reviewed source replacement remains eligible for queue reactivation.'
    ],
    decisions: {
      evidence_archive_review_history_contract_v5: {
      ...
      evidence_archive_maintenance_queue_v6: {
        decision: 'approved_internal_after_pr397',
        pr: 398,
    ...
    activation_rule: 'PR #397 must update AGENTS.md and docs/roadmap.md before generating History v5 outputs.',
    boundaries: {
...
export function writePostPr395ReviewGate(report = buildPostPr395ReviewGate()) {
...
if (isCli) {
  const report = buildPostPr395ReviewGate();
  ...
  } else writePostPr395ReviewGate(report);
  console.log(JSON.stringify({
    ...
    history_v5: {
      sources: report.expected_history_v5.sources,
      events: report.expected_history_v5.events,
      identities: report.expected_history_v5.identities,
      archive_present: report.expected_history_v5.effective_archive_present,
      invalid_removed: report.expected_history_v5.effective_invalid_removed,
      no_safe_change: report.expected_history_v5.effective_no_safe_change,
      source_replacement: report.expected_history_v5.effective_source_replacement,
      unresolved: report.expected_history_v5.reviewed_unresolved_total,
      suppressed: report.expected_history_v5.reviewed_suppressed,
      reactivated: report.expected_history_v5.reviewed_reactivated_eligible
    },
