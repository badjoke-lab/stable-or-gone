...
check(Object.keys(implementedMobileTableRepresentations).length === 1, 'exactly one page-specific mobile representation must be implemented at PR 24');
check(implementedMobileTableRepresentations['stablecoin-index'] === 'src/components/StablecoinIndexCard.astro', 'stablecoin mobile representation source changed');
...
check(audit.totals?.implemented_mobile_representations === 1, 'stablecoin mobile representation was not detected');
check(JSON.stringify(audit.current_baseline?.implemented_mobile_table_kinds) === JSON.stringify(['stablecoin-index']), 'implemented mobile table kinds changed');
...
check(gaps.table_transformations_pending?.length === 24, 'page transformation queue must contain 24 pending tables after stablecoin index completion');
check(!gaps.table_transformations_pending?.includes('stablecoin-index'), 'stablecoin index must not remain in the transformation queue');
...
const validation = {
  ...
  totals: {
    ...
    page_families: 8,
    table_contracts: 25,
    implemented_mobile_representations: 1,
    global_shell_gaps: 0,
