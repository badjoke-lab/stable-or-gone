...
for (const [device, config] of Object.entries(devices)) {
  ...
  if (metrics.logos < 1) deviceFailures.push('no direct Stablecoin/product logo is visible');
  if (metrics.fallbacks < 1) deviceFailures.push('no neutral fallback is visible');
  if (metrics.logos + metrics.fallbacks !== metrics.visibleMarks) deviceFailures.push('visible mark kinds do not account for every record');
