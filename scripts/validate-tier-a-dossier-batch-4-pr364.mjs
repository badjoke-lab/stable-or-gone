...
equal(queue.items.map((row) => row.asset_slug), selected, 'review queue selected assets changed');
check(queue.selected_count === 5, 'review queue selected_count must remain five');
check(queue.queue_order === 'asset_slug_ascending_non_ranking', 'review queue order changed');
check(queue.canonical_promotion_authorized === false && queue.manual_review_required === true, 'review queue promotion boundary changed');
