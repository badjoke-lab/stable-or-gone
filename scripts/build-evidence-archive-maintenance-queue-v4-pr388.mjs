...
const paths = {
  config: 'config/evidence-archive-maintenance-queue-v4-pr388.json',
  ...
  priorQueue: 'docs/migration/evidence-archive-maintenance-queue-v3-pr383.json'
};
const outputPaths = {
...
export function buildEvidenceArchiveMaintenanceQueueV4Outputs() {
  ...
  if (authority.decisions?.evidence_archive_maintenance_queue_v4?.pr !== 388) {
    throw new Error('PR #386 does not authorize PR #388');
  }
  if (authority.decisions?.evidence_archive_maintenance_queue_v4?.decision !== 'approved_internal_after_pr387') {
    throw new Error('PR #386 Queue v4 decision changed');
  }
  if (historyAudit.decision?.next_work_item !== 'PR #388 Evidence Archive Maintenance Queue v4 Refresh') {
    throw new Error('PR #387 audit does not hand off to PR #388');
  }
  if (contract.contract_id !== 'sog_evidence_archive_review_history_v3_pr387') {
    throw new Error(`Unexpected archive review-history contract ${contract.contract_id}`);
  }
  if (historyManifest.manifest_id !== 'sog_evidence_archive_review_history_manifest_v3_pr387') {
    throw new Error(`Unexpected archive review-history manifest ${historyManifest.manifest_id}`);
  }
  if (historyAudit.audit_id !== 'sog_evidence_archive_review_history_audit_v3_pr387_2026_07_16') {
  ...
  const selectedCandidates = selected.map(({ row, priority, history, reactivated, selectionTier }) => ({
    ...
    review_reasons: [
      ...
      reactivated ? 'reviewed_reactivated_history_v3' : 'unreviewed_archive_gap',
      priority.bucket
    ],
  ...
  const queue = {
    schema_version: '4.0',
    queue_id: 'sog_evidence_archive_maintenance_queue_v4_pr388',
    status: 'reviewed_internal_non_ranking_history_v3_queue',
    public_output: false,
    evidence_rank: false,
  ...
  const delta = {
    schema_version: '3.0',
    delta_id: 'sog_evidence_archive_maintenance_queue_v4_delta_pr388',
    status: 'reviewed_internal_archive_queue_history_v3_delta',
    public_output: false,
    review_pr: 388,
    reviewed_at: config.reviewed_at,
    prior_queue_id: priorQueue.queue_id,
...
export function writeEvidenceArchiveMaintenanceQueueV4Outputs(outputs = buildEvidenceArchiveMaintenanceQueueV4Outputs()) {
...
if (isCli) {
  const outputs = buildEvidenceArchiveMaintenanceQueueV4Outputs();
  ...
  } else writeEvidenceArchiveMaintenanceQueueV4Outputs(outputs);
