...
const queuePath = 'docs/migration/evidence-archive-maintenance-queue-v4-pr388.json';
const configPath = 'config/evidence-archive-maintenance-batch-5-pr390.json';
const outputPath = 'docs/migration/evidence-archive-maintenance-batch-5-pr390-review-queue.json';
...
if (JSON.stringify(selectedIds) !== JSON.stringify(config.selected_evidence_ids)) {
  throw new Error('PR #390 config selected IDs do not match PR #388 queue order');
}
...
const output = {
  schema_version: '1.0',
  queue_id: 'sog_evidence_archive_maintenance_batch_5_pr390_review_queue',
