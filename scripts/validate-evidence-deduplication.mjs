...
const expectedPublicSourceIdentities = expectedEvidenceRecords - evidenceSourceAliasCount;
...
assert(evidenceSourceIdentityGroupCount === groups.length, `approved identity groups must match exact duplicate URL groups: ${evidenceSourceIdentityGroupCount} vs ${groups.length}`);
assert(evidenceSourceAliasCount === canonicalRecords.length - publicSources.length, 'source alias count must explain the public-source reduction');
...
for (const group of groups) {
  ...
  for (const key of ['stablecoin_ids','organization_ids','event_ids','claim_scopes']) assert(Array.isArray(group.relation_union?.[key]), `${group.group_id}: ${key} relation union is missing`);
}
...
for (const approvedGroup of evidenceSourceIdentityGroups) {
  ...
  for (const key of ['stablecoin_ids','organization_ids','event_ids','claim_scopes']) assert(sameArray(unique(projected[key] ?? []), unique(auditGroup.relation_union?.[key] ?? [])), `${approvedGroup.canonical_id}: ${key} union was not preserved`);
}
...
for (const source of publicSources) {
  ...
  if (url) publicUrlCounts.set(url, (publicUrlCounts.get(url) ?? 0) + 1);
}
...
assert(groupedEvidenceIds.size === evidenceSourceAliasCount + evidenceSourceIdentityGroupCount, 'grouped evidence count must equal canonical identities plus aliases in approved duplicate groups');
assert(publicSources.reduce((sum, source) => sum + (source.source_alias_ids?.length ?? 0), 0) === evidenceSourceAliasCount, 'public source alias total mismatch');
...
fs.writeFileSync(path.join(root, 'data/generated/evidence-deduplication-validation.json'), `${JSON.stringify(validation, null, 2)}\n`);
