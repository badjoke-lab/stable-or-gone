...
const requiredUsage = {
  ...
  'src/components/StablecoinIndexRow.astro': [
...
const forbiddenUsage = {
  'src/components/StablecoinDetailView.astro': ['const primaryRelationship = relationships[0]'],
  'src/pages/stablecoins/index.astro': ['getPrimaryRelationship']
};

const failures = [];
for (const [file, requiredTerms] of Object.entries(requiredUsage)) {
  const source = fs.readFileSync(file, 'utf8');
  for (const term of requiredTerms) if (!source.includes(term)) failures.push(`${file}: required primary-display usage is missing: ${term}`);
}
for (const [file, forbiddenTerms] of Object.entries(forbiddenUsage)) {
  const source = fs.readFileSync(file, 'utf8');
  for (const term of forbiddenTerms) if (source.includes(term)) failures.push(`${file}: array-order or legacy primary selection remains: ${term}`);
}
if (failures.length > 0) throw new Error(failures.join('\n'));
console.log(JSON.stringify({ ok: true, protected_surfaces: Object.keys(requiredUsage).length, forbidden_array_order_patterns: Object.values(forbiddenUsage).flat().length }, null, 2));
