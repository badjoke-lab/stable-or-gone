...
function values(rows, selectors) {
  const set = new Set();
...
const axes = {
  lifecycle_status: values(stablecoins, [(row) => row.lifecycle_status]),
  legacy_status: values(stablecoins, [(row) => row.status]),
  issuance_status: values(stablecoins, [(row) => row.issuance_status]),
  asset_class: values(stablecoins, [(row) => row.asset_class]),
  ...
  reference_asset: values(stablecoins, [(row) => row.peg_reference?.asset, (row) => row.peg_asset]),
  backing_type: values([...stablecoins, ...classifications], [(row) => row.backing_types]),
  stabilization_mechanism: values([...stablecoins, ...classifications], [(row) => row.stabilization_mechanism]),
  governance_model: values([...stablecoins, ...classifications], [(row) => row.governance_model]),
