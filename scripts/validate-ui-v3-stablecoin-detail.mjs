...
for (const marker of [
  'class="stablecoin-dossier-masthead"',
  'class="stablecoin-dossier-title-row"',
  'class="stablecoin-dossier-facts"',
  'class="stablecoin-material-change"',
  'class="stablecoin-dossier-nav"',
  'class="stablecoin-assessment"',
  'class="stablecoin-identity-ledger"',
  'class="stablecoin-mechanism-ledger"'
]) check(source.view?.includes(marker), `Editorial Ledger dossier marker is missing: ${marker}`);

for (const label of [
  'Stablecoin dossier', 'Record ', 'Also recorded as', 'Lifecycle', 'Issuance', 'Reference target',
  'Primary organization', 'Launch', 'Redemption', 'Backing', 'Stabilization', 'Last review',
  'Latest material change', 'Evidence'
]) check(source.view?.includes(label), `required upper information is missing: ${label}`);
...
for (const anchor of ['#assessment', '#organizations-control', '#mechanism', '#reserves-redemption', '#deployments-legal-context', '#history', '#evidence', '#known-unknowns', '#more']) {
  check(source.view?.includes(`href="${anchor}"`), `local navigation destination is missing: ${anchor}`);
}

for (const label of ['Name', 'Symbol', 'Aliases', 'Asset class', 'Lifecycle status', 'Issuance status', 'Canonical record ID', 'Route slug', 'Record confidence', 'Last reviewed']) {
  check(source.view?.includes(`<dt>${label}</dt>`), `identity field is missing: ${label}`);
}
...
for (const marker of [
  '.stablecoin-dossier-masthead', '.stablecoin-dossier-facts', '.stablecoin-material-change',
  '.stablecoin-dossier-nav', '.stablecoin-dossier-section', '.stablecoin-section-heading',
  '.stablecoin-identity-ledger', '.stablecoin-mechanism-ledger', '.stablecoin-value-state-group',
  '.stablecoin-further-reading', '@media (max-width: 719px)', '@media (forced-colors: active)'
]) check(source.styles?.includes(marker), `detail style marker is missing: ${marker}`);
...
const result = {
  ...
  page_family: 'editorial_ledger_research_dossier',
