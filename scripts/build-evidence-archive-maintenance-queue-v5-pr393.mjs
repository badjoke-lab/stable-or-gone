...
const paths = {
  config: 'config/evidence-archive-maintenance-queue-v5-pr393.json',
  ...
  priorQueue: 'docs/migration/evidence-archive-maintenance-queue-v4-pr388.json'
};
const outputPaths = {
...
export function buildEvidenceArchiveMaintenanceQueueV5Outputs() {
  ...
  if (authority.decisions?.evidence_archive_maintenance_queue_v5?.pr !== 393) throw new Error('PR #391 does not authorize PR #393');
  if (authority.decisions?.evidence_archive_maintenance_queue_v5?.decision !== 'approved_internal_after_pr392') throw new Error('PR #391 Queue v5 decision changed');
  if (historyAudit.decision?.next_work_item !== 'PR #393 Evidence Archive Maintenance Queue v5 Refresh') throw new Error('PR #392 audit does not hand off to PR #393');
  if (contract.contract_id !== 'sog_evidence_archive_review_history_v4_pr392') throw new Error(`Unexpected contract ${contract.contract_id}`);
  if (historyManifest.manifest_id !== 'sog_evidence_archive_review_history_manifest_v4_pr392') throw new Error(`Unexpected manifest ${historyManifest.manifest_id}`);
  if (historyAudit.audit_id !== 'sog_evidence_archive_review_history_audit_v4_pr392_2026_07_16') throw new Error(`Unexpected audit ${historyAudit.audit_id}`);
  ...
  if (historyManifest.counts.history_source_count !== input.history_source_count || historyManifest.counts.history_event_count !== input.history_event_count) throw new Error('History v4 source/event count changed');
  if (historyManifest.counts.reviewed_evidence_identity_count !== input.reviewed_identity_count) throw new Error('History v4 identity count changed');
  if (historyAudit.reviewed_unresolved.suppressed_count !== input.reviewed_suppressed_count) throw new Error('History v4 suppression count changed');
  if (historyAudit.reviewed_unresolved.reactivated_eligible_count !== input.reviewed_reactivated_eligible_count) throw new Error('History v4 reactivation count changed');
  ...
  const selectedCandidates = selected.map(({ row, priority, history, reactivated, selectionTier }) => ({
    ...
    review_reasons: ['archive_not_recorded', reactivated ? 'reviewed_reactivated_history_v4' : 'unreviewed_archive_gap', priority.bucket],
  ...
  const queue = {
    schema_version: '5.0',
    queue_id: 'sog_evidence_archive_maintenance_queue_v5_pr393',
    status: 'reviewed_internal_non_ranking_history_v4_queue',
    public_output: false,
    evidence_rank: false,
  ...
  const delta = {
    schema_version: '4.0',
    delta_id: 'sog_evidence_archive_maintenance_queue_v5_delta_pr393',
    status: 'reviewed_internal_archive_queue_history_v4_delta',
    public_output: false,
    review_pr: 393,
    reviewed_at: config.reviewed_at,
    prior_queue_id: priorQueue.queue_id,
...
export function writeEvidenceArchiveMaintenanceQueueV5Outputs(outputs = buildEvidenceArchiveMaintenanceQueueV5Outputs()) {
...
if (isCli) {
  ...
  } else writeEvidenceArchiveMaintenanceQueueV5Outputs(outputs);
