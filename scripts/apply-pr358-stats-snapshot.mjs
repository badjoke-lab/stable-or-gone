...
if (snapshot.totals?.assets !== 112 || snapshot.totals?.evidence !== 557 || snapshot.totals?.market_access_records !== 4 || snapshot.totals?.deployments !== 174) {
  throw new Error(`Unexpected PR #358 totals: ${JSON.stringify(snapshot.totals)}`);
}
...
if (existingIndex >= 0) {
  ...
  if (JSON.stringify(existing) !== JSON.stringify(snapshot)) throw new Error('Existing PR #358 snapshot differs from deterministic output.');
